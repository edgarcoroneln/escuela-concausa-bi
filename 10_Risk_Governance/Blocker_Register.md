...
| BLOCK-003 | US-304a / US-305 / BUG-025 / REQ-006 | Decisión de arquitectura sin tomar (PM) | Célula 3 (Andrés) · Célula 4 (Christian) · Célula 5 (Luis) | **No hay proveedor ni modelo de LLM acordado, y las tres células están detenidas en el mismo punto:** Andrés no puede implementar `generar_sql()`/`redactar_respuesta()` sin saber contra qué cliente programa; Luis no sabe qué dependencia ni qué secreto añadir a la imagen; Christian no sabe qué formato consumir. El agente vale **0.5 pts** y es el único de los siete módulos que hoy no se demuestra de punta a punta | 2026-08-30 | **Decidir hoy cinco cosas:** proveedor, modelo, nombres de variables de entorno, dependencia Python y límites de tokens/timeout. **Mitigación si no hay acuerdo:** el agente se demuestra con el guardarraíl funcionando —ya en `main`, sí rechaza SQL destructivo— y la generación se declara deuda de S5 | Edgar Coronel (decide) · Andrés González (implementa) | open |
