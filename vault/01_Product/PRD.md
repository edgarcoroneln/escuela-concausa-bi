...
> ([[vault/01_Product/PRD_General_Materia]], `PRD-GENERAL`, que define **QUÉ** nos piden).
> Todo `REQ-###` en [[vault/02_Requirements/Requirements_Detailed]] traza a una sección de aquí.
...
parte prescriptiva (ML-02, el driver dominante). Ver [[vault/01_Product/OKRs_NorthStar]].
...
diseño del modelo de datos. Ver [[vault/07_Security/_index]].
...
Detalle en [[vault/03_Architecture/Data_Model]] y [[vault/03_Architecture/API_Specification]].
...
> navegación y catálogo de KPIs) vive en [[vault/04_UX_Design/Screen_Specs]].

### 12.1 Capa web integrada (FARO Web)

Los 10 dashboards se integran en **FARO Web**, una app **Streamlit** (`src/frontend/`) que además hospeda el **panel de ML interactivo**, el **widget de chat del agente** y el **login/logout con vistas protegidas por rol**. Los dashboards se embeben por **guest token con row-level security**. Cubre el §3.5 del PRD de la materia. Historias: US-206, US-207 (C2), US-405 (C4), US-305 (C3). Ver [[vault/03_Architecture/Frontend_Architecture]] y [[vault/03_Architecture/ADRs/ADR-002-frontend-streamlit]].
...
> Recordatorio de la rúbrica externa ([[vault/01_Product/PRD_General_Materia]]): **sin URL pública
...
|---|---|---|
| Rendimiento | Respuesta de API y dashboards aceptable en demo en vivo | [[vault/08_CICD_DevOps/CI_Quality_Gates]] |
| Seguridad | OAuth2/JWT + RBAC; sin secretos en el repo | [[vault/07_Security/_index]] |
| Accesibilidad | Dashboards legibles, contraste y etiquetas claras | — |
| Disponibilidad / SLO | URL pública estable durante la ventana de evaluación | [[vault/11_Operations/Monitoring_SLOs]] |
| Privacidad | Dato mínimo = escuela (CCT); cero PII de alumnos | [[vault/07_Security/_index]] |

---

## 16. Referencias

- Requerimiento externo (QUÉ): [[vault/01_Product/PRD_General_Materia]] (`PRD-GENERAL`)
- Faro y OKRs: [[vault/01_Product/OKRs_NorthStar]]
- Personas y casos de uso: [[vault/01_Product/Personas]]
- Modelo de datos y APIs: [[vault/03_Architecture/Data_Model]] · [[vault/03_Architecture/API_Specification]]
- Riesgos: [[vault/10_Risk_Governance/Risk_Register]]
- Roadmap y fases: [[vault/12_Roadmap_Sprints/Roadmap]]
