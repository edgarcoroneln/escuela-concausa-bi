...
| **Formato de entrega** | Rama fija `dev/marina-garcia` (sincronizada con `main`) → PR con plantilla y título estándar → 1 aprobación (PM) → merge a `main` |
...
| **Cómo se entrega** | Rama fija `dev/marina-garcia` (sincronizada con `main`) → PR con plantilla → 1 aprobación del PM → merge a `main` |
...
Trabajas siempre en **`dev/marina-garcia`**. No creas una rama por historia, ni por sprint, ni por tema: esa
...
# 4. Sube
git push origin dev/marina-garcia

# 5. Abre el PR en GitHub con la plantilla (se carga sola) y este título:
#    [Marina Garcia] - Descripción concisa (US-###) - [sync|CI|DoF|DevLog]
...
`YYYY-MM-DD-marina-garcia-descripcion.md`. Debe registrar: qué pediste, qué generó la IA, qué revisaste
...
- Tu alcance vive en dos lugares que dicen lo mismo: `vault/_Meta/ownership.yml` (lo que lee el CI) y
  tu `vault/09_AI_Governance/Agent_Contexts/marina-garcia-agent-context.md` (la versión legible).
