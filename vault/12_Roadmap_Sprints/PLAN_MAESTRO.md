...
> (URL viva) y a los gates de readiness de [[vault/13_Reports/PM_Dashboard_Spec]].
...
2. **Ramas:** una fija y permanente por persona, `dev/{primer-nombre}-{apellido-paterno}`.
   Se sincroniza con `git merge origin/main`; nunca `rebase`, nunca `--force`, y no se borra al mergear.
...
10. **No apruebas tu propio PR.** Una aprobación, la del PM (DEC-003).
11. **Solo tocas tu alcance**, definido en `vault/_Meta/ownership.yml`. El CI lo verifica.
...
| `RISK-003` | Commits concentrados en pocas personas | Rama fija por persona (`dev/{identidad}`), PR obligatorio, revisión del PO en cada standup |
