...
- **Estado:** in_progress — reabierto por `DEC-022`: documentar componentes, capas, filtros y cubos con evidencia apta para el storytelling (`US-601`…`US-603`).
...
- **Estado:** in_progress — reabierto por la evaluación del profesor: rediseño UX/UI, gráficas, storytelling e integración del frontend (`US-621`…`US-624`, `US-641`).
...
- **Estado:** in_progress — ML-03 debe funcionar en Gold/API/UI y la explicación de los tres modelos debe demostrar métricas, límites y valor (`US-631`…`US-633`).
...
- **Estado:** in_progress — reabierto para documentar componentes y validar la integración frontend/API/chat/Auth (`US-601`, `US-612`, `US-643`).
...
- **Estado:** in_progress — las URLs del 8-sep son baseline; falta desplegar, observar y validar la nueva candidata con rollback (`US-642`).
...
- **Estado:** in_progress — el profesor no aceptó la interacción actual; se requiere conversación natural, contexto, guardarraíles y evidencia desplegada (`US-611`…`US-613`).
...
- **Estado:** in_progress — gobernanza, QA y trazabilidad de la recuperación siguen abiertos hasta la nueva entrega (`US-006`, `US-603`, `US-651`…`US-655`).
