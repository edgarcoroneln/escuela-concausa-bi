...
| BUG-025 | El endpoint desplegado `/api/v1/agente/consulta` era el **stub** de `src/api/v1/agente.py`: respondía la misma cadena fija a cualquier pregunta, incluidas las fuera de alcance y destructivas. Además su filtro buscaba `"borrar"` por subcadena, así que **«Borra la tabla de predicciones»** no disparaba el rechazo | high | **fixed** | US-304a, US-305, REQ-006 | `src/api/v1/agente.py` delega en `procesar_consulta()` con recuperador, LLM, ejecutor y redactor inyectables; `src/api/app.py` cablea ejecutor read-only y LLM sólo cuando hay configuración. El endpoint ya no devuelve la respuesta hardcodeada del stub, corta órdenes destructivas antes del LLM y degrada sin filtrar detalle si falta configuración. El E2E autenticado en nube sigue pendiente de credenciales/despliegue, pero ya no es este bug de stub | `tests/test_agente_endpoint.py` · `tests/test_agente_wiring_llm.py` |
...
| BUG-037 | `sync_semantic_layer.py` actualiza el SQL de un dataset (`PUT /api/v1/dataset/<id>`) pero nunca vuelve a pedirle a Superset que re-lea sus columnas: la lista de columnas queda fija desde la creación del dataset. Cualquier columna agregada, renombrada o quitada en un `.sql` de `superset/semantic/` después de esa primera creación se sincroniza en el texto del SQL pero no en el esquema del dataset — cualquier chart que la referencie falla con `Columns missing in dataset: [...]`, y el error solo aparece al abrir el dashboard en el navegador, nunca en la corrida del script ni en la API de creación de charts (que no valida contra el schema real) | medium | open | US-214b / REQ-002 | **Dueño asignado 2026-09-05: Monserrat Xcaret Miranda Olivas** (antes decía "pendiente de dueño de `sync_semantic_layer.py`", y llevaba desde el 30-ago sin responsable pese a figurar en el runbook de ambiente local como problema conocido). Criterio: en C2 el script compartido lo corrige **quien encuentra el defecto**, con Manuel Serranía como reviewer por ser dueño de la convención (US-202) — precedentes: Marina García en BUG-049, Oscar Quiroz en el filtro de ciclo, y la propia autora en BUG-038. Además nació de US-214b, vive en su verde y ella misma propuso el fix. **Programado para después de la demo del 9-sep, a propósito**: es `medium`, tiene mitigación documentada, y tocar la herramienta compartida de los 10 tableros mientras C5 corre el bootstrap a producción arriesga más que el propio defecto. Mitigación vigente: `PUT /api/v1/dataset/<id>/refresh` a mano tras el sync. Fix de fondo: que `ensure_datasets()`/`_apply_metrics_and_columns()` llame ese endpoint automáticamente cada vez que el SQL de un dataset cambia | Reportado por Monserrat Miranda (2026-08-30), validando US-214b en vivo: `db05_cubo_driver` ganó la columna `link_db08`, se sincronizó el SQL, y el chart "D1 · Municipios · driver dominante y cobertura" reventó con 4 columnas de esa fila (3 preexistentes + la nueva) marcadas "missing" hasta refrescar a mano. Sin test de regresión automatizado todavía |
| BUG-038 | `_layout_tabs()` en `sync_semantic_layer.py` arma `ROOT_ID` con `type: "TABS"` y cuelga los 6 nodos `TAB-D1..D6` directamente como sus hijos. Superset renderiza el contenido del primer tab (D1) correctamente, pero **no dibuja ninguna barra de navegación entre tabs** — D2 a D6 quedan inalcanzables desde la interfaz, aunque sus datos y charts existen y funcionan bien vía API. El test `test_layout_tabs_arma_root_de_tipo_tabs` codifica esta estructura como si fuera la esperada (`assert position["ROOT_ID"]["type"] == "TABS"`), por eso pasaba en verde sin que nadie lo notara — nunca se había confirmado visualmente en un navegador real hasta hoy. **Segundo síntoma, misma causa probable**: los filtros globales de DB-05 (Ciclo/Entidad/Nivel) se pueden editar en el panel (quitar un valor, "Clear all") pero el cambio nunca le llega a los charts — ni al quitar un valor y darle "Apply filters", ni con "Clear all". Probado directo contra `/api/v1/chart/data` (sin pasar por el navegador): el mismo filtro por `cve_ent` sí funciona perfecto ahí (filtrar a `'09'` trae solo Ciudad de México, filtrar a `'19'` trae solo Nuevo León) — el SQL/dataset está bien, el defecto es que el *scope* del filtro nativo (`rootPath: ["ROOT_ID"]`) probablemente no puede resolver qué charts están "dentro" del árbol cuando ese árbol está mal armado para tabs, el mismo defecto de fondo que el síntoma anterior | high | fixed | US-213 / US-214b / REQ-002 | **Corregido 2026-09-04 (Monserrat Miranda).** Eran **dos** defectos y había que arreglarlos juntos — por eso las dos hipótesis previas fallaron, cada una tapaba uno: (1) `ROOT_ID` se declaraba `type: "TABS"`, cuando Superset espera `ROOT` con el contenedor de tabs como nodo aparte; sin él no dibuja la barra. (2) se interponía un `GRID-<id>` entre cada `TAB` y sus `ROW`, y con tabs las filas cuelgan directo del TAB — con el GRID en medio el contenido no se renderiza. El árbol quedó `ROOT_ID(ROOT) → TABS-ROOT(TABS) → TAB-<id> → ROW → CHART`. Cambio aditivo: `_layout_grilla()` y los 8 tableros del camino plano no se tocaron (suite completa 886 passed / 0 failed) | **Sí, y reescrito porque mentía.** `test_layout_tabs_arma_root_de_tipo_tabs` afirmaba `ROOT_ID.type == "TABS"`: codificaba el defecto como esperado, por eso el sync pasaba en verde con el tablero roto. Eran **3** los tests que fijaban la estructura defectuosa (no 1, como decía este registro) más 3 que dependían del GRID. Ahora hay guarda explícita por defecto: `test_layout_tabs_root_es_root_y_cuelga_del_contenedor_de_tabs` y `test_layout_tabs_sin_grid_intermedio_entre_tab_y_filas` (54 casos en `test_semantic_db05_db08.py`). **Verificado en navegador real** contra Superset 6.1.0, no sólo por API — es la lección del bug: los 6 tabs navegables con contenido propio (D1 52.7 %/18, D4 30.9 %/17) y los filtros globales llegando a los charts (Jalisco → 0 de 7, contrastado contra `gold.cubo_driver`). Evidencia en [[vault/06_Quality_Testing/Usability_Accessibility_Test_Plan_DB05_DB08]] §1.2/§1.3 |
...
| BUG-048 | **Producción sirve un Gold empobrecido: `indice_completitud_drivers` 0.197 y `escuelas_en_riesgo` 0, contra un pipeline local con drivers completos y salidas ML defendibles.** El snapshot importado a Cloud SQL se generó antes de BUG-045; el código desplegado era correcto y los datos estaban viejos. Dentro de esta misma alta, Diana Alvarez (C1) destapó y cerró dos huecos de completitud no vistos hasta entonces: `dbt/models/sources.yml` seguía apuntando `bronze.sesnsp` (D2/inseguridad) y `bronze.conapo` (D1/D2, denominador poblacional) a sus fixtures (`sesnsp_test`, `conapo_sample` 36 filas) pese a existir cargador y dato real — **consolidado aquí tras el merge de `origin/main`: esta fila fusiona lo que antes eran dos altas separadas de BUG-048 (PM/C3 y C1), mismo ID, mismo bug** | **high** | **fixed** | US-113 / US-122b / US-302 / US-311 / US-313 / US-505 / REQ-001 / REQ-003 / REQ-005 / DS-07 | **C1 ✅ 2026-09-05 (Diana Alvarez), los dos huecos cerrados con evidencia real:** SESNSP — default corregido a `'sesnsp'` (`165c911`), `silver.delitos_municipio` cruza 2,486 municipios, `comp_media` D2 estable. CONAPO — nuevo `src/ingesta/cargar_bronze_conapo_real.py` (transforma a largo el parquet real de Emilio Galnares, suma sexo, valida contra `POB_TOTAL`) + default corregido a `'conapo'` (`19fecc4`): `comp_media` D2 sube de ~0.48 a ~0.62 y `d2_sin_dato` cae de ~36,700 a **0** en los 3 ciclos. De paso, `gold.cubo_pipeline` destrabado con datos reales de DS-06/CONAGUA (180 presas → 11 filas). **C3 ✅** ML-01 con `loss="absolute_error"` mejora MAE temporal 11.04 % sobre baseline; ML-02 F1 0.8331; dump final `gold_bug048_final_2026-09-05.sql` (19 tablas, incluye `cubo_pipeline`) generado y restaurado de prueba, mismo dump entregado a C3/C5. **C5 ✅ 2026-09-05 (Luis Téllez):** desplegado a producción. Backup Cloud SQL `1788644444768` → reset (`DROP SCHEMA gold CASCADE`, exit 0, `faro_app` es owner) → import server-side del dump `final2` (`gold_bug048_final2_2026-09-05.sql`, 66 MB, SHA-256 `b8a3fc50…a5b32a`, evaluado read-only en contenedor desechable y confirmado con Andrés antes de importar; exit 0, 8 matviews refrescadas). **Verificado en vivo vía API** (imagen `33fcbbb`, sin redeploy — es solo dato): `indice_completitud_drivers` **0.197 → 0.619** (3×), `matricula_total` 6,704,229 (ciclo vigente), 317 municipios; diferenciador **vivo** (09DBN0007I→D4/conectividad vs 09DAL0009J→D2/seguridad, recomendación distinta). `escuelas_en_riesgo` sigue **0** por umbral 0.6 > max riesgo 0.572 (H1, decisión de C2/C3, no defecto del dump). Ver [[vault/_DevLog/2026-09-05-luis-tellez-bug048-refresco-gold-prod]]. **BUG-048 cerrado.** | `tests/test_entrenar_ml01.py` 34 passed · `tests/test_publicar_gold.py` 39 passed · dbt 158/159 (único desfase, ajeno a C1: 1,168 recomendaciones de features fuera de `fact_escuela_ciclo`) · `dbt test --select poblacion_municipio+` tras el fix de CONAPO: 218 passed / 2 errors (staleness del ML runtime de C3 sobre el Gold recién reconstruido, autoresuelve cuando C5/Andrés reimporten — no es regresión de C1) · restauración aislada: 45,276 predicciones, 45,276 recomendaciones, cinco drivers, ocho cubos, riesgo 0.0106–0.8744 · **Validacion independiente (Diana Alvarez, 2026-09-06):** `gold_bug048_final2_2026-09-05.sql` (run `bug048-20260905-temporal-robusto`) confirma 7/45,276 escuelas con `indice_riesgo >= 0.5` (avg 0.351030, max 0.571702) antes de rematerializar Gold con DEC-019 — coincide al digito con lo reportado por Marina Garcia desde produccion; ver DevLog 2026-09-06 |
| BUG-049 | **Las tarjetas y tablas de DB-03/DB-04 quedan alineadas al fondo de su celda, con hueco vertical arriba, y las tablas anchas exigen scroll horizontal para ver las últimas columnas.** Encontrado en la sesión de usabilidad de US-215a (Marina García, 2026-09-04, Chrome): los `big_number_total` dejan un espacio vacío notable bajo la cifra, y en `Perfil del plantel`, `KPI-16` y `KPI-18` las columnas de la derecha —incluidas las de link, que son el drill-down de US-214a— quedan fuera de vista hasta desplazar. **No afecta los datos ni la navegación**, ambos verificados correctos; es presentación, y el tablero es material de la demo del 9-sep | low | **fixed** | US-215a / US-212 / REQ-002 | **Causa raíz encontrada y corregida (Marina García, 2026-09-05), y NO era ajustar alturas.** `_layout_grilla()` en `superset/sync_semantic_layer.py` ponía **cada chart en su propia fila**, así que el `ancho` declarado en los YAML no servía para nada: cuatro tarjetas de `ancho: 3` —escritas para ir lado a lado y sumar 12— se apilaban una debajo de otra, cada una ocupando 3/12 con nueve doceavos vacíos a su derecha. Verificado contra el tablero desplegado antes de tocar nada: DB-03 tenía **11 charts en 11 filas**. Nuevo `_agrupar_en_filas()` agrupa consecutivos mientras quepan en los 12 de la grilla, respetando el orden declarado. **Resultado: DB-03 pasa de 11 filas a 5, DB-04 de 13 a 5**, y los 7 tableros que usan este layout dejan de ser tiras verticales. Con eso las columnas de link de US-214a quedan alcanzables sin scroll largo, que era el efecto práctico del defecto | `tests/test_layout_filas_bug049.py` (18 casos): el caso de las 4 tarjetas de ancho 3, el patrón real de DB-03/DB-04, e **invariantes** que valen para los 8 tableros — ninguna fila excede 12, no se pierde ni duplica ningún chart, se respeta el orden, y el árbol `ROOT_ID → GRID_ID → ROW → CHART` con `parentId` correcto. Se actualizó una aserción incidental de `test_semantic_db01_db02.py::test_el_layout_genera_estructura_v2` (de Manuel), que codificaba el comportamiento defectuoso |
| BUG-050 | **Lighthouse reporta score de accesibilidad 93 en DB-03, con dos hallazgos: contraste insuficiente entre texto y fondo, y `<html>` sin atributo `[lang]`.** Medido por Marina García el 2026-09-04 en Chrome, idéntico en modo claro y oscuro. El del `lang` **no es de estos tableros**: lo emite el shell de Superset, así que corregirlo sería configuración de la imagen (C5) o un upstream, no un cambio de tablero. El de contraste sí puede tocar a los tableros, pero **no hay paleta declarada contra la cual verificar**: `vault/04_UX_Design/UX_Guidelines.md` está en `status: draft` con las tablas de tokens vacías pese a llevar `source_of_truth: true` **Medición fina 2026-09-05 (Marina García), que es la que DEC-016 exige adjuntar:** medido con `getComputedStyle` elemento por elemento sobre color y fondo **efectivos**, en DB-03 y DB-04 y en los dos temas, en vez de con el score agregado. **Reprueban exactamente dos elementos, los dos del shell de Superset**: `Published` `#5AC189` sobre `#F0FFF4` = **2.16:1**, y `Edit dashboard` `#2893B3` sobre `#E4F1F2` = **3.07:1** en claro y sobre `#1A3A44` = **3.41:1** en oscuro. **Todo lo que escribe FARO pasa**: peor caso 19.26:1 en claro y 18.42:1 en oscuro. Contexto útil para dimensionarlo: de los 10 colores primarios del esquema de fábrica `supersetColors` que pintan los 103 charts, **8 reprueban 4.5:1 sobre blanco y 5 no llegan ni a 3:1** — extraídos del bundle que corre, no de documentación. **Por DEC-016 deja de bloquear `US-215a`**, que cierra 22/22; sigue `open` como deuda declarada. **Al medir por elemento apareció un defecto propio que el score escondía** (links de drill-down a 3.26:1 en claro), corregido aparte — ver caso 3.1.bis del plan de pruebas. | low | open | US-215a / REQ-002 | pendiente — **dos dueños distintos**: el `[lang]` es del shell de Superset (**C5**, `docker/superset.Dockerfile` / `superset_config.py`); el contraste requiere primero que alguien llene la paleta de `UX_Guidelines.md` (**C2 · Manuel Serranía**, dueño del sistema de diseño). Sin paleta no hay criterio de aceptación | ⬜ **el proyecto declara un gate que no existe**: `vault/04_UX_Design/Accessibility.md` dice *"verificados en CI (Lighthouse a11y)"* y *"Lighthouse Accessibility ≥ 0.9 (bloqueante)"*, pero no hay ninguna referencia a Lighthouse en `.github/` ni en `vault/08_CICD_DevOps/`. El 93 se midió a mano |
| BUG-051 | La **etiqueta del tab activo de DB-05** (`D1 · Pobreza y rezago social`) queda en **4.07:1** de contraste en tema oscuro y **3.55:1 en tema claro** sobre su fondo, ambos por debajo del mínimo AA de 4.5:1 para texto normal (14 px, no es texto grande). **Está peor en claro que en oscuro**, así que no se resuelve eligiendo un tema por defecto. Medido sobre el color y el fondo **efectivos** del nodo, no los declarados. Es el color de acento del tab seleccionado de Superset, no un color propio del tablero — pero el texto lo introdujo US-213, y es el control con el que se navegan los 6 drivers, así que se registra en vez de descartarlo como "de un tercero". En el mismo barrido, **30 de 32 nodos cumplen AA en oscuro y 31 de 34 en claro**; los demás fallos son chrome de Superset, explícitamente fuera del alcance del plan: `Edit dashboard` (3.41 oscuro / 3.07 claro) y `Published` (2.16, sólo claro). La etiqueta del tab es **el único fallo que es contenido del tablero** | low | open | US-215b / US-213 / REQ-002 | pendiente — no se corrige por cuenta propia: tocar el acento de los tabs afecta a **todos** los tableros, no sólo DB-05, así que es decisión de la convención de la capa semántica (Manuel Serranía, US-202) y del sistema de diseño. Nota estructural: `vault/04_UX_Design/UX_Guidelines.md` está **vacío** con `source_of_truth: true`, así que hoy no existe una paleta oficial contra la cual ratificar un color de reemplazo | Reportado por Monserrat Miranda (2026-09-04) verificando §3.1 de US-215b en navegador real, tras el fix de BUG-038. Medido en **ambos temas**. Sin test de regresión: el proyecto no tiene CI de accesibilidad (hueco ya documentado en el propio plan de pruebas) |
| BUG-052 | **`tests/test_frontend_dashboards_streamlit.py` es intermitente: falla ~2 de cada 6 corridas sin que cambie una línea del árbol, y no siempre falla la misma prueba.** Síntoma idéntico en las dos: `app.sidebar.selectbox` vacío donde deberían estar los filtros — la página no llega a esa sección. **Causa raíz documentada en el propio fixture** (`superset_fake`, líneas 66-71): `AppTest` comparte `sys.modules` entre `.run()`, así que la constante `SUPERSET_URL` de `superset_client` queda congelada del run anterior; si el test previo usó otro puerto efímero —cuyo servidor ya se apagó— el siguiente conecta a una URL muerta y la página degrada. La purga de módulos que el fixture ya hace **mitiga pero no cierra**: cuál de las dos pruebas hereda el módulo viejo depende del orden y del momento. El fixture limpia `cache_data` pero **no `cache_resource`**. **La fragilidad estaba documentada antes de este reporte**: el docstring del fixture `autouse` de `tests/test_frontend_panel_ml_streamlit.py` (línea 34) ya dice *"la fragilidad de esa suite es previa — se reproduce en el commit anterior a este"*. Medido por **Marina García (C2)** el 2026-09-05 sobre `f2ee9d2`, worktree limpio, mismo intérprete, 6 corridas seguidas: **2 fallaron**. Reproducción en Windows / Python 3.11.9 / Streamlit 1.63.0; **no reproducido en macOS** —el PM corrió 12 veces seguidas y pasaron las 12— ni medido en Linux, así que la tasa fuera de Windows no se afirma | medium | open | US-206 / US-214b / REQ-002 | pendiente (**C2 — Manuel Serranía**, dueño de `src/frontend/**`). Patrón de arreglo **ya existente en el repo**: el fixture `autouse` de `test_frontend_panel_ml_streamlit.py` limpia `cache_data` **y** `cache_resource` antes de cada prueba, no sólo al entrar y salir del fixture. **No lo introduce ni lo arregla el PR #234**: verificado que entre `f2ee9d2` y su punta no hay un solo cambio en ese archivo de pruebas ni en `src/frontend/**` | ⚠️ **Por qué el CI en verde no lo desmiente:** el job corre `pytest tests/ -q` **una sola vez** por PR. Una corrida verde de algo que falla 1 de cada 3 no prueba determinismo, prueba que esa vez tocó pasar — el PM cometió justo esa inferencia al declararlo "no reproducible" tras una sola corrida. **Riesgo con el freeze encima:** puede poner en rojo el CI del PR de cualquiera y hacerle perder una hora buscando en su propio cambio |
| BUG-053 | **`GET /api/v1/predicciones/{cct}/explicacion` no devuelve valores SHAP: las `contribuciones` son los seis drivers de `mock_data`, no la salida de ningún modelo.** Y no es un pendiente de C4: **hoy no existe fuente que leer**. `src/modelos/entrenar_ml02.py:320::explicar_driver` calcula SHAP con la forma exacta de `ExplicacionSHAPOut` —está bien hecha— pero **no la llama nadie** (verificado con `grep` en todo `src/`: el único hit es su definición, más su propia prueba), y `publicar_gold.py` sólo escribe `gold.predicciones` y `gold.recomendaciones` — ninguna guarda contribuciones. Calcularlo por petición tampoco es opción: `shap` vive en `requirements/celula-3.txt` y no está en la imagen de la API, y `KernelExplainer` tarda segundos por fila, incompatible con el `statement_timeout` y la degradación 503 de US-416. **Alcance de la demo:** el driver dominante y la recomendación de `/predicciones/{cct}` **sí son reales** (salen de ML-02); lo que es mock es el desglose de contribuciones. El diferenciador del proyecto se sostiene, el detalle fino no. Diagnosticado por Christian Ruiz (C4) el 2026-09-05 al ir a conectarlo; verificado de forma independiente por el PM | medium | fixed | US-412 / US-302 / REQ-003 / REQ-004 | **Cerrado 2026-09-05 (Christian Ruiz, C4).** El endpoint lee `gold.recomendaciones.shap_d1..shap_d6` via `RepositorioModelos`, ya no `mock_data`. Se **reutiliza la fila de `obtener_prediccion`** en vez de anadir una consulta propia: hereda el cache TTL y la traduccion a 503 de US-416, y hace imposible que la explicacion se desincronice del `driver_dominante` que dice explicar. Se anade `?ciclo` (antes asumia uno, el mismo default silencioso de BUG-044). `null` se transporta como `null`, nunca como `0.0` -- eso seria reintroducir BUG-055 con datos reales. Depende de `924c8b4` (Andres Gonzalez, C3), que persiste las columnas nullable | `tests/test_explicacion_shap.py` (15 casos): contribuciones desde el repositorio, las seis claves siempre presentes, `null` sobrevive en D5/D6, **un `0.0` medido no se confunde con un hueco**, signo negativo preservado, 404 por CCT y por ciclo, 503 con Gold caido, y la matriz de acceso `require_lectura` |
| BUG-055 | *(registrado como `BUG-053` en su rama; renumerado por `DEC-013` al llegar a `main` — ese ID ya lo ocupaba el mock de SHAP del mismo endpoint, que es otro defecto: distinta causa, distinto dueño, y sigue `open` mientras éste cierra `fixed`.)* **`GET /predicciones/{cct}/explicacion` devuelve `0.0` donde no hay dato, afirmando que un driver contribuyó cero cuando en realidad no se sabe.** El endpoint hacía `escuela.get(f"d{i}") or 0.0` y el contrato era `dict[str, float]`, que **no puede representar el hueco**. No es teórico: `mock_data` ya modela cobertura parcial y la app respondía `15DPR0100B → D5: 0.0` (dato real: `None`) y `14DPR0250D → D6: 0.0` (`None`) — justo los dos drivers que el proyecto sabe parciales, D5 (estrés hídrico, regional) y D6 (aire, ~80 zonas urbanas), donde el hueco es el caso **normal**. Contradice la regla de cobertura parcial del proyecto (*nunca cero, nunca nulo silencioso*), que los cubos y `indice_completitud_drivers` sí respetan. El `or` además colapsaba un `0.0` medido y un hueco en el mismo número, volviendo indistinguible un driver irrelevante de uno nunca evaluado — con SHAP real (US-302) eso corrompería la respuesta a la pregunta central del proyecto: *por qué* está en riesgo cada escuela. Impacto acotado: la ruta no tiene consumidores todavía, por eso se corrige **antes** de que C3 persista SHAP, cuando ensanchar el contrato aún no rompe a nadie. Hallado por Christian Ruiz (TL C4) el 2026-09-05 preparando la conexión con C3 | medium | fixed | US-412 / US-302 / REQ-004 | `contribuciones` pasa a `dict[str, float \| None]`, se elimina el `or 0.0`, las seis claves siguen siempre presentes (el hueco se declara, no se omite). `API_Specification` §3.4/§7 actualizada con la instrucción para C3: **persistir nulos, no ceros** | `tests/test_explicacion_shap.py` — `test_un_driver_sin_dato_viaja_como_none_no_como_cero[D5,D6]` y `test_un_cero_legitimo_no_se_confunde_con_un_hueco`; **las 3 reprueban con el `or 0.0` anterior**, comprobado |
| BUG-056 | **Las 122 celdas de valor del pivote de DB-08 (`pvtVal`) quedan en 3.55:1 sobre blanco en tema claro**, por debajo del mínimo AA de 4.5:1. Superset les aplica **en línea** el azul de acento `#2893b3`; verificado que `db08_explorador_cubo.yaml` **no declara ningún color ni formato condicional** —sólo estructura (`groupbyRows`, `metricsLayout`, `rowTotals`…)—, así que el color es **heredado del tema, no estilizado por FARO**: por **DEC-016** se registra como limitación conocida con su medición y **no bloquea** US-215b. **Sólo ocurre en claro**: en oscuro el mismo acento sobre el fondo del tablero da 1208 de 1209 elementos en AA. Se registra aparte de BUG-051 —que es la etiqueta del tab activo de DB-05— porque es otro tablero y otro elemento, aunque compartan el color de origen. **Pesa más que el chrome**: son los valores del explorador, el contenido central del tablero, no un botón de la barra | low | open | US-215b / US-213 / REQ-002 | pendiente — mismo techo que BUG-050/BUG-051: corregir el acento de Superset es cambiar el tema o un *upstream*, y eso vive en REQ-005, no en REQ-002 (DEC-016). Si alguna vez FARO adopta paleta propia, este es uno de los puntos a re-medir | Reportado por Monserrat Miranda (2026-09-05) al ejecutar §3.1 del plan de usabilidad **sobre DB-08**, que hasta esa fecha sólo tenía evidencia de DB-05. Sin test de regresión: el proyecto no tiene CI de accesibilidad (hueco ya documentado en el propio plan) |
| BUG-057 | **La URL pública dejó de servir datos: `AUTH_LECTURA_PUBLICA` pasó a `false` en producción y toda ruta de lectura responde 401.** Medido por el PM el 2026-09-05 ~23:45: `/api/v1/kpis`, `/api/v1/escuelas` y `/api/v1/predicciones/{cct}` devuelven **401**, cuando minutos antes devolvían **200** sin credenciales. `/api/v1/health`, `/version` y `/docs` siguen en 200. **La imagen no cambió** —`/version` sigue en `33fcbbb`—, así que fue un cambio de variable de entorno en Cloud Run, no un redespliegue. **Puede ser deliberado**: `src/api/config.py:72` describe exactamente este paso —*"cuando C5 entregue credenciales, se pone `AUTH_LECTURA_PUBLICA=false`"*— y C5 entregó los usuarios de prueba hoy. Se registra igual porque **cambia lo que el evaluador ve**: con Superset ya en SSO obligatorio, **ambas superficies exigen login y la URL pública no sirve un solo dato de forma anónima**, y la rúbrica evalúa *URL pública viva* (`RISK-001`, techo de 6.0). No es un defecto de código: `require_lectura` (`src/api/security/rbac.py:61`) se comporta como está especificado | **high** | closed | US-403 / US-411 / US-412 / US-501 / REQ-004 / REQ-005 | **CERRADO reflejando decisión del PO (postura a), no por código — 2026-09-05.** La **postura de acceso (a)** —sostener OAuth/login en las **dos** superficies (API + Superset), con la lectura cerrada— es **decisión del PO (Edgar Coronel)**, como este registro la marcó desde su alta; el PO la asienta como **DEC-018**. **Luis Téllez (C5) se inclinó por (a)** y **ejecutó** el flip técnico (`AUTH_LECTURA_PUBLICA=false`) como **cierre de SEC-006** —esa ejecución sí es de C5: la condición de cierre estaba escrita desde el 2026-09-02, es el paso que `src/api/config.py:72` anticipa (*"cuando C5 entregue credenciales, se pone `AUTH_LECTURA_PUBLICA=false`"*)— por env-var en Cloud Run (rev `faro-api-00012-pq5`, imagen `33fcbbb` intacta, reversible a `=true`). Este cierre es **documental**: refleja la decisión del PO y el hallazgo de C5, sin cambio técnico. La *URL pública viva* de la rúbrica **se mantiene** (verificado en vivo 2026-09-05): `/api/v1/health`, `/api/v1/version` (commit `33fcbbb`) y `/api/v1/docs` + `/api/v1/openapi.json` (Swagger) siguen anónimos en **200**; `RISK-001` está **cerrado** — DEC-012 acota el criterio de URL pública a las rutas HTTP de la API, no a servir el `gold` sin login; OAuth+RBAC en ambas capas **suma** (Backend/Auth 1.5 + Deploy 1.0) | ✅ **Acceso del evaluador resuelto sin tocar env de la API:** el profesor entra a la API como **ciudadano** (rol por defecto de `resolve_role`, `src/api/security/roles.py`) — cualquier cuenta Google que complete OAuth obtiene ≥ciudadano y puede leer; la API **no** tiene lista blanca de login (la única lista, `redirect_uri`, es anti-open-redirect, no de personas), así que **NO** hay que darlo de alta en `ANALISTA_EMAILS`. La única compuerta por persona es **Superset** (whitelist fail-closed `SUPERSET_SSO_ALLOWED_EMAILS`): el correo del profesor **ya está** en la lista (rev `faro-superset-00006-68n`). **Asimetría de acceso que el hallazgo destapa (la asienta el PO en DEC-018):** con la lectura cerrada, **cualquier** cuenta de Google —no solo el evaluador— obtiene rol `ciudadano` y puede leer la API; **Superset sí tiene lista blanca estricta, la API no**. Para datos **agregados** de escuela (privacidad por diseño: la unidad observada es la escuela, nunca el alumno) es defendible, pero la asimetría queda **explícita** y no descubierta por alguien más el miércoles. Checklist pre-demo de Luis: confirmar ese correo en la whitelist de Superset y, si la consent screen sigue en *Testing*, darlo de alta como *test user* en la consola Google. Ver [[vault/07_Security/Security_Audit_Log]] SEC-006 (`resolved`) y `AUTH_LECTURA_PUBLICA=false` en vivo |
| BUG-054 | **Dos defectos en las métricas de DB-09, el tablero del diferenciador. (1) *Recomendaciones de prioridad ALTA* muestra `No data` cuando hay 2:** `metrics_db06_db09.yaml:217` compara `prioridad = 'ALTA'` en mayúsculas y el cubo la guarda en minúsculas, así que el `CASE` no empata ninguna fila y el `SUM` sobre el vacío devuelve `NULL`. Verificado contra `gold.cubo_recomendaciones`: **alta 2, media 29, baja 24** y 90 nulas legítimas; la expresión tal cual devuelve `NULL` y con `upper(prioridad)` devuelve **2**. **La tarjeta que dice a qué escuelas entrar primero sale vacía.** **(2) *% escuelas con recomendación* sólo puede dar 0 % o 100 %:** `metrics_db06_db09.yaml:222` divide `SUM(recomendacion_emitida)` entre `COUNT(DISTINCT cct)` y **ambos son `bigint`**, así que Postgres hace **división entera**. Hoy muestra 100 % **por coincidencia** —en 2024-2025 las 55 escuelas tienen recomendación y 55/55 = 1—; con una sola que no la tuviera, **54/55 daría 0** y la tarjeta caería de 100 % a 0 % sin pasar por nada intermedio (98.2 % se mostraría como 0 %). Los otros dos ciclos muestran 0 %, que ahí **sí es correcto**, y eso es justo lo que tapa el error. Es el modo de falla de `BUG-017` y `BUG-031`: un número creíble que significa otra cosa. **Alcance verificado por el PM: es la ÚNICA métrica de toda la capa semántica con este defecto** — se sondearon las 19 divisiones con `pg_typeof` contra la base y las otras 18 devuelven `numeric`, porque `SUM(bigint)` promueve a `numeric` y sólo `recomendacion_emitida` es `smallint` (casteado en `cubo_recomendaciones.sql:42`), cuyo `SUM` sí devuelve `bigint`. Reportado por Marina García (C2) el 2026-09-05 revisando las capturas del gate visual, fuera de su historia | **high** | **fixed** | US-204 / US-214b / REQ-002 | **Corregido 2026-09-05 (Manuel Serranía, C2).** `metrics_db06_db09.yaml:217` → `SUM(CASE WHEN upper(prioridad) = 'ALTA' THEN 1 END)`; `:222` → `SUM(recomendacion_emitida)::numeric / NULLIF(COUNT(DISTINCT cct), 0)`. Guardas de regresión agregadas en `tests/test_semantic_db06_db09.py` (exige ambas expresiones fijas). **Pendiente del re-sync de la capa semántica** para reflejarse en los tableros — se ejecutará con el próximo sync/bootstrap | ⬜ **Re-sync + reconfirmar en navegador antes del 9-sep** (aplica porcentaje real: con 2 `alta`, la tarjeta prioridad pasa de `No data`→2; y el % puede mostrar fracciones, no saltar 0↔100). Nota: mi alta anterior de este bug atribuía el `No data` a la tarjeta equivocada — es la de prioridad ALTA, no la de porcentaje. Corregido con la medición fina de Marina. Y **dos métricas del repo ya llevan el `* 1.0` defensivo** (`metrics_db03_db04.yaml:70`, `metrics_db06_db09.yaml:145`): alguien conoció esta trampa, se defendió de ella en dos lugares donde los operandos ya eran seguros, y no la propagó al único sitio donde sí muerde |
| BUG-058 | **El KPI-04 «escuelas en riesgo» da 0 estructuralmente: el umbral de alerta (índice ≥ 0.60 = perder ≥ 5 % de matrícula, `DEC-006`) queda por encima de lo que el fenómeno alcanza en educación básica.** Doble evidencia, **no es un ajuste cosmético**: (a) empírica — la mayor caída NETA que ML-01 predice sobre el Gold real de producción es **4.53 %** (máx `indice_riesgo` **0.5717**, ver `ML01_Entrenamiento` §"Resultados sobre Gold real"), así que el conteo es 0 **por construcción**; (b) de negocio — la deserción nacional real es **0.6 % primaria / 3.7 % secundaria** (ciclo 2023-2024), muy por debajo del 5 %: un umbral de 5 % describe una crisis, no el punto donde una alerta **temprana** debe encender. No es defecto de código ni del modelo (ambos correctos): es **utilidad/calibración del umbral de alerta**, hoy pegado al ancla de la sigmoide pero **separable** de ella. **Dos salidas para que C2 pondere:** **(B1, para la demo)** bajar solo la línea de alerta del KPI (p. ej. índice 0.45–0.50 ≈ perder 2.6–3.4 %) **sin recalibrar la sigmoide** — los `indice_riesgo` no cambian, no se re-publica Gold ni se re-entrena; **(D, estratégica)** **poblar el universo con media superior**, donde la deserción es **11.3 %** y el umbral de 5 % aplica tal cual — pero hoy el pipeline **excluye media superior por diseño** (`dbt/models/silver/matricula_historica.sql` filtra a PREESCOLAR/PRIMARIA/SECUNDARIA; `NIVELES_BASICA` en `src/ingesta/validacion_cct.py` y `cargar_bronze_cct_real.py`) y **CEMABE no cubre ese nivel** (`DS-03_CEMABE.md` §"Cobertura … no media superior"), así que D3/D4 entrarían `SIN_DATO` estructural → es trabajo de **C1 + C3**, no de C5, y **fuera del CODE FREEZE**. Medición del universo aportada por C5 (prod, 45 276 escuelas): **≥0.60 → 0** · ≥0.50 → 7 · ≥0.40 → 11 775 (26 %) · ≥0.35 → 24 951 (55 %). Hallado en la demo del Mapa de Riesgo (KPI-04 = 0), 2026-09-06 | medium | open | US-215a / US-204 / DEC-006 / REQ-002 | **pendiente — Marina García del Buey (C2): validar y decidir el umbral de alerta** (con Manuel Serranía —TL C2—, Christian Ruiz por el filtro `indice_riesgo >= 0.6` en `src/api/repositorio_gold.py`, y firma del PO); la decisión se asienta como **DEC-019** (revisión de `DEC-006`). La salida **D (media superior)** queda propuesta como **historia futura de C1 + C3** (extender el sensor a EMS, con la salvedad de CEMABE en D3/D4). **C5 solo aporta la medición; no ejecuta el cambio de umbral ni la ampliación.** No mover a `done` sin la decisión firmada | pendiente — al fijar el umbral: test que verifique conteo > 0 sobre fixtures con el nuevo corte. Nota: `tests/test_riesgo.py::test_reproduce_el_ancla_del_umbral_de_negocio` valida la **sigmoide**, que B1 **no toca** → sigue verde · **2026-09-06 hito (C2, Manuel Serranía): «B1» ejecutado en la presentación** — con `DEC-019` firmado y los cubos Gold ya contando `>= 0.5` (PR #273, C1), `umbral:` de `escuelas_en_riesgo` → 0.5 (los dos datasets) y `subheader` de DB-02/DB-09 → 0.5; la nota de KPI-04 cita `DEC-019`. Estado sigue `open` **solo hasta el re-sync del ambiente** (cubos ya re-materializados por C1; falta sincronizar Superset para el cierre visual) = guarda nueva `tests/test_umbral_alineado.py` |
| BUG-059 | *(registrado como `BUG-058` en su rama; renumerado por `DEC-013` al llegar a `main` -- ese ID ya lo ocupaba el umbral estructural del KPI-04, que entro antes)* **FARO Web perdia la sesion a los 15 minutos y las paginas internas no tenian encabezado de sesion.** Tres defectos en `src/frontend/auth.py`, reportados por el PO el 2026-09-06: **(1)** `auth.py` guardaba el `refresh_token` y **nunca lo usaba** -- cero llamadas a `/auth/refresh` en todo el front -- asi que el access token (15 min, `config.py:33`) moria a media demo: iniciar sesion en la preparacion significaba llegar sin sesion a la sala. **(2)** `logout_button()` vivia solo en `app.py:25`: quien entraba directo a Dashboards, Panel de ML o Chat no veia con quien estaba dentro ni podia cerrar sesion. **(3)** `2_Panel_ML.py:142` mandaba `(user or {}).get('access_token')`, pero `/auth/me` devuelve `sub`/`email`/`name`/`role` y **nunca** el token: era `None` siempre. No se notaba porque la lectura es publica; el dia que dejara de serlo, la pagina daria 401 con sesion iniciada. `3_Chat.py:34` leia la clave directo, mismo problema por otra puerta | **high** | fixed | US-405 / US-207 / US-305 / REQ-004 | **Corregido 2026-09-06 (Christian Ruiz, C4).** `token_de_acceso()` refresca contra `/auth/refresh` con 120 s de margen usando el `expires_in` que la API ya devolvia y el front tiraba; si la API rechaza el refresh se cierra la sesion (mejor 'Inicia sesion' que un token muerto dando 401 en cada pagina), pero un corte de red **no** borra la sesion. `encabezado()` unifica el bloque de sesion: una linea por pagina. Las paginas ya no leen `st.session_state['access_token']` directo. **No se cambio quien puede entrar**: el Panel de ML sigue de lectura publica | `tests/test_frontend_auth.py` 12 -> **20 casos**; las 3 del refresco **reprueban con el codigo anterior** (comprobado). 23 passed en la suite de frontend |
| BUG-060 | **El camino mock y el camino real del KPI-04 usaban umbrales distintos, y ninguna prueba podía cazarlo.** `src/api/mock_data.py:152` cuenta `escuelas_en_riesgo` con `indice_riesgo >= 0.5` **desde antes de `DEC-019`**, mientras `repositorio_gold.py:325` contaba con `>= 0.6`. Es decir: el mismo KPI daba números distintos según hubiera base de datos o no, y **cada camino se probaba contra sí mismo**, así que la divergencia era invisible para la suite. Detectado por **Christian Ruiz** al implementar `DEC-019` y reportado por **Diana Alvarez**. | medium | **fixed (por coincidencia, y por eso se registra)** | DEC-019, BUG-058, REQ-004 | `DEC-019` baja el camino real a 0.50 y **con eso los dos coinciden — pero por accidente, no por diseño**: el mock estuvo tres semanas «correcto» por casualidad y habría vuelto a divergir al siguiente cambio de umbral. El arreglo de fondo es `RISK-010`: una sola fuente para el corte, post-freeze. Christian ya lo dejó importando la constante (`from src.api.repositorio_gold import LINEA_DE_ALERTA`) en su rama, que es la mitad del camino | **Falta la guarda → añadida el 2026-09-06 (C2, Manuel Serranía).** `tests/test_umbral_alineado.py` exige que el `umbral:` de `escuelas_en_riesgo` en el YAML de presentación sea `LINEA_DE_ALERTA` (0.5) y que los `subheader` de DB-02/DB-09 y la nota de KPI-04 no vuelvan a imprimir 0.6; `tests/test_linea_de_alerta.py` (PM/Christian) ya cubre el flanco mock vs real. La divergencia entre caminos ya no puede reaparecer invisible. **Ampliada el 2026-09-06 (2.ª ronda, PR #275, respuesta al PM):** la guarda pasa de 4 archivos a **barrido total de `superset/semantic/**` y `superset/dashboards/**`** (YAML, SQL y READMEs) — falle ante cualquier 0.6/0.60 asociado a riesgo salvo el ancla (`ANCLA_SIGMOIDE` citado junto a 'ancla'/'calibra'). Se corrigieron los 6 `umbral: 0.6` residuales (`metrics_db03_db04`, `metrics_db06_db09`), el corte ejecutable `p.indice_riesgo >= 0.6` de `db09_cubo_recomendaciones.sql`, la banda ALTA del mock y los comentarios de cabecera de db02/db03/db04/db06 + READMEs. El `subheader` del KPI-17 de DB-03 pasa de «desde 0.60 (DEC-006)» a «desde 0.50 (DEC-019)» *por indicación del PM*: la etiqueta visible describe la línea de alerta; el ancla sigue viva solo en `LINEA_DE_ALERTA`/`ANCLA_SIGMOIDE` (Python) y en la documentación de calibración |
| BUG-061 | **`main` no puede reproducir lo que corre en producción: el embebido de Superset de FARO Web nunca se commiteó.** `src/frontend/superset_client.py` tiene **188 líneas en `main`** y la versión que sirve la URL pública tiene **265**; `1_Dashboards.py` con el Embedded SDK está en la misma situación. El código vivía **sólo en el working tree local** y llegó a producción horneado en una imagen vía `COPY src/frontend/`. Detectado por **Luis Téllez** al reconstruir desde `main` para bajar `BUG-059`: la reconstrucción **regresó Dashboards y Panel ML**, porque trajo el `superset_client.py` viejo. | **high** | open | US-206, US-526, REQ-002, REQ-005 | **Dueño: Célula 2** (`src/frontend/**` es su alcance; C5 sólo conteneriza). Commitear el embebido a `main` desde el handoff que Luis dejó en `_local/`. **Mitigación temporal de C5**: la revisión `00005-6vs` en Cloud Run ya sirve la versión buena, así que **hoy la demo funciona** — pero cualquier `rebuild` desde `main` la regresa | **Riesgo directo sobre el 9-sep:** un redespliegue por cualquier motivo —un fix de última hora, un rollback, un reinicio que dispare build— deja los tableros embebidos sin autenticar **sin que nadie lo note hasta abrirlos**. Verificado por el PM: `main` = 188 líneas, y la URL `https://faro-frontend-eanzfglvyq-uc.a.run.app` responde **200** |
| BUG-062 | **`driver_dominante` compara drivers normalizados sobre poblaciones distintas, así que el argmax premia al driver con cobertura más angosta.** En `dbt/models/gold/features_escuela.sql` **cada driver se reescala min-max sobre su propio conjunto cubierto**: D1 sobre los municipios con rezago y D2 sobre los que tienen tasa —cobertura casi nacional—, pero **D6 sobre las escuelas dentro de los 15 km de una estación SINAICA**, que es ~1.3 % del universo (`where distancia_km <= 15`, CTE `dentro_radio_aire` → `d6_rango` → `d6`). El min-max estira **siempre** su entrada hasta llenar el `[0,1]` sin importar qué tan angosto sea el rango de origen, así que en el subconjunto que sí tiene aire los valores de D6 salen inflados frente a los D1/D2 **de esas mismas escuelas**, y ganan el argmax con más frecuencia de la que su severidad justifica. Consecuencia: la etiqueta es en parte un artefacto de **amplitud de cobertura**, no de gravedad. **Síntoma observado** (Marina García del Buey, 2026-09-06, sobre el Gold de producción): D6 es dominante en **419 de 45 276** escuelas (0.93 %) pero en **12 de las 40** de mayor riesgo — El Oro (7) y Temascalcingo (5), municipios rurales. Concentración ~32× la esperada. | medium | open | US-302 / US-113 / REQ-001 / REQ-003 | **NO se corrige antes del 9-sep, y es decisión del PO.** Rematerializar `features_escuela` cambia el `driver_dominante` de todo el universo, y con él **todas** las predicciones publicadas, el par de demostración de `US-006` (`15DPR0920D` / `15DPR2254O`) y el conteo de 7 escuelas que sostiene `DEC-019`. Mismo criterio con el que se difirió `BUG-037`: severidad media, causa entendida, y el arreglo mueve más suelo que el defecto. **Dueño: Célula 1 (Diana Aracely Alvarez Varela)** por ser dueña de `dbt/**`, **con Célula 3 (Andrés González Habib)** como acuerdo de la especificación del argmax (`US-302`, 2026-08-28). **Descartada la hipótesis de imputación** con la que se reportó: el filtro `distancia_km <= 15` deja fuera del CTE a la escuela sin estación cercana, y el ensamblado la marca `coalesce(d6.d6_cobertura, 'SIN_DATO')`. La regla de `SIN_DATO` **sí se respeta**; el defecto es de comparabilidad de escalas, no de relleno. **Fix de fondo propuesto:** normalizar los seis drivers sobre un referente común (percentil nacional por driver, o un rango fijo por unidad física) en vez de min-max por cobertura, y declarar el `driver_dominante` sólo cuando el `indice_completitud_drivers` supere un mínimo. **Mitigación para la demo:** está escrito en el guion como respuesta preparada; no se ordena por riesgo descendente sin poder explicarlo. | — (propuesto: prueba dbt que compare la distribución de cada `dN_*` normalizado y falle si el rango efectivo de uno difiere del de los demás más allá de una tolerancia) |
| BUG-063 | **La tarjeta «Recomendaciones de prioridad ALTA» de DB-09 lee 0 en producción: el corte de `prioridad` quedó por encima del techo del fenómeno.** `src/modelos/publicar_gold.py:197` asigna `Prioridad.ALTA` sólo si `riesgo >= ANCLA_SIGMOIDE` (**0.60**), y el máximo que ML-01 predice sobre el Gold real de producción es **0.5717** (`DEC-019`, medido de forma independiente por C2 y C5). Ninguna de las **45 276** escuelas alcanza esa categoría, así que la columna `prioridad` de `gold.recomendaciones` no tiene una sola fila `alta`. `superset/dashboards/db09_recomendaciones.yaml:51` expone una tarjeta llamada literalmente **«Recomendaciones de prioridad ALTA»** (métrica `recomendaciones_prioridad_alta`, `metrics_db06_db09.yaml:215`) más un gráfico «Recomendaciones por prioridad» desglosado por esa columna: **la tarjeta que dice a qué escuelas entrar primero muestra cero, y la barra de ALTA no existe** — en el tablero del diferenciador. **Es el mismo defecto estructural de `BUG-058`** —un corte de negocio por encima del techo del fenómeno— **en otra columna**: `DEC-019` bajó la línea de alerta del KPI-04 a 0.50 y arregló el conteo, pero **no tocó `prioridad`**. **Por qué `BUG-054` no lo cazó:** ese bug corrigió la comparación de mayúsculas (`upper(prioridad)`, ya en `main`) y se verificó contra un fixture de ~55 filas donde sí había *«alta 2, media 29, baja 24»*. El fix es correcto; lo que cambió es el universo — en 45 276 filas ese 2 es 0. | medium | open | US-204 / US-313 / DEC-019 / DEC-006 / REQ-002 / REQ-003 | **NO se corrige antes del 9-sep. Decisión del PO.** Detectado por **Héctor Rafael Morales Marbán** al implementar `DEC-019` en la capa de modelos (PR #279): documentó que **no** bajó el corte a 0.50 porque reescribiría la columna `prioridad` de las 45 276 filas ya publicadas, y `DEC-019` dice explícitamente que no cambia un solo valor publicado. Hizo bien en no decidirlo solo; ésta es la respuesta. **Tres salidas evaluadas:** (1) no mostrar DB-09 el miércoles — el guion no lo pide por nombre; (2) alinear `prioridad` a la línea de alerta y republicar — coherente, pero reescribe 45 276 filas a dos días de la entrega y contradice `DEC-019`; (3) dejarlo y **decirlo antes de que lo pregunten**, igual que con `escuelas_en_riesgo`. **Se elige la (3):** es la línea que el guion ya usa y no mueve el suelo bajo los números que se van a mostrar. **Fix de fondo, post-demo:** decidir si `prioridad` debe seguir el **ancla de la sigmoide** (calibración, 0.60) o la **línea de alerta** (criterio de negocio, 0.50) — son dos cosas distintas desde `DEC-019` y hoy usa la primera sin que nadie lo haya decidido. Dueño: **C3 (Héctor Morales, con Andrés González Habib como TL)** junto con el PO. | — (propuesto, y es la guarda que habría cazado también `BUG-058`): una prueba que contraste el corte de la categoría más alta contra el **máximo observado en el Gold publicado** y falle cuando ninguna fila pueda alcanzarla. Un umbral inalcanzable por construcción no es una opinión de negocio: es un defecto detectable |
| BUG-064 | **DB-01 en producción: leyenda de pastel paginada/truncada y eje X del driver dominante sin etiquetas.** Tres síntomas del mismo tablero, agrupados por ser todos de legibilidad, no de dato: **(1)** "Escuelas por nivel (KPI-08)" (pastel, 3 categorías) muestra la leyenda **paginada** — solo 1 de 3 categorías visible (`◀ 1/3 ▶`), hay que darle clic para ver el resto; **(2)** el mismo pastel y "Escuelas por sostenimiento (KPI-09)" truncan las etiquetas de rebanada ("SECU...", "PR...", "PRI...", "PÚB...") — en el primero, "PR..." es **ambiguo** entre "Primaria" y "Preescolar", ambas empiezan igual; **(3)** "Driver dominante (KPI-07)" (barras, 6 drivers) solo muestra el nombre de la categoría bajo la primera barra ("Agua"); las otras 5 barras (valores 419, 12,765, 2,086, 2,792, 26,046, 6) no traen su nombre de driver en el eje X — es el tablero que muestra el diferenciador del proyecto y hoy nadie puede saber qué representa cada barra sin el eje. **Los datos son correctos en los tres casos** — es presentación, no un valor mal calculado. Detectado por **Oscar Antonio Quiroz Lázaro (C2)**, QA pre-demo en producción, sesión `analista`, 2026-09-07 ~20:43-20:44 CST — ver [[vault/06_Quality_Testing/QA_Logs/2026-09-07-oscar-quiroz-qa-pre-demo]] casos 3-5. **Ampliado el mismo día:** el mismo patrón de leyenda paginada (solo 1 categoría visible de varias, con `◀ N/M ▶`) se repite en DB-06 · Predicciones, "Distribución del riesgo proyectado" (pastel/donut) — cuarto síntoma del mismo defecto de legibilidad, ahora confirmado en dos tableros (DB-01, DB-06) | **medium** | open | US-203 / REQ-002 | **decidido 2026-09-08:** dueño C2 (código compartido de Célula 2, `superset/dashboards/db01_ejecutivo.yaml` + `sync_semantic_layer.py`), **reviewer Manuel Serranía**, fix post-9 (sync congelado, criterio BUG-037). **No corregir corriendo `sync_semantic_layer.py` antes del 9-sep** (regla de ventana vigente: el sync puede borrar el parche de 20 charts que Luis aplicó a mano, PR #275) — cualquier fix debe esperar a después de la demo o coordinarse explícitamente con el PO | pendiente — ningún test cubre hoy legibilidad de leyendas/ejes en los charts declarativos de `superset/dashboards/*.yaml` |
| BUG-065 | **DB-03 en producción: el filtro "Escuela (CCT)" no se puede aplicar para `15DPR0920D` — uno de los dos CCT oficiales del par de demostración (`US-006`) — porque aparece duplicado en la lista del dropdown.** `15DPR0920D` sale dos veces en las opciones del `filter_select` (ambas entradas marcadas ✓ al seleccionarlo), y con esa selección el botón "Apply filters" queda deshabilitado sin importar el reintento — no hay forma de filtrar el tablero a esa escuela desde la UI. **Aislado, no es el filtro en general:** con un CCT de control sin duplicar (`01DPR0540S`) "Apply filters" se habilitó normalmente y las 4 tarjetas cambiaron a valores de esa escuela (28 alumnos, -22.2%, 67%, con KPI-17 en "No data" — comportamiento esperado, esa escuela no tiene predicción ML-01). **Efecto sin el filtro aplicado:** las 4 tarjetas de "Ficha de Escuela" (KPI-15/02/17/05, etiquetadas "de la escuela") muestran el agregado nacional completo (matrícula 6,704,229, variación -2.9%, índice 0.36, completitud 62%) — idéntico a los totales de DB-01/DB-02 — como si una sola escuela tuviera 6.7 millones de alumnos. **Bloquea una verificación de pre-demo real:** el plan de pruebas (Diana) exige confirmar que `15DPR0920D` y `15DPR2254O` dan `índice_riesgo = 0.4774` idéntico entre sí; con este bug no se pudo verificar el primero. Detectado por **Oscar Antonio Quiroz Lázaro (C2)**, QA pre-demo en producción, sesión `analista`, 2026-09-07 ~21:05-21:10 CST — ver [[vault/06_Quality_Testing/QA_Logs/2026-09-07-oscar-quiroz-qa-pre-demo]] casos 11-15. **Escalado por Teams a Edgar Coronel de inmediato**, antes de escribirse aquí, por ser 🔴 (riesgo directo sobre el minuto 3:00-5:00 del guion de la demo) | **critical** | open | US-212 / US-214a / US-006 / REQ-002 | Sin dueño asignado todavía — pendiente de instrucción del PO tras la escalación. Hipótesis sin verificar en código: valor duplicado en la dimensión CCT que alimenta el `filter_select` de `db03_ficha_escuela.yaml`, o un problema de estado del componente de filtro ante dos opciones con la misma llave | pendiente — sin prueba de regresión todavía; el caso 13 de la bitácora de QA (CCT de control sin duplicar) es la evidencia de aislamiento, no una prueba automatizada |
| BUG-066 | **DB-04 en producción: el eje Y de índices 0-1 no muestra decimales y el eje X pierde el nombre de casi todos los municipios — mismo patrón de causa que `BUG-064`.** En "KPI-03 · Riesgo promedio por municipio" y los mini-gráficos D1/D2 (`echarts_timeseries_bar`, sin filtro de municipio), el eje Y muestra "0, 0, 0, 1" en vez de una escala decimal (0, 0.2, 0.4... 1.0) — con un índice que vive entre 0 y 1, ese redondeo hace parecer que casi todos los municipios tienen riesgo/driver ≈0 cuando en realidad está distribuido en todo el rango. En los mismos 7 gráficos (KPI-03 y D1-D6), el eje X solo muestra la etiqueta del primer municipio alfabético ("Abasolo"); las ~300 barras restantes no traen nombre. **Causa raíz acotada al código, no al YAML de Marina:** `_params_chart()` en `superset/sync_semantic_layer.py` sí toma correctamente el `formato: decimal_2` (`",.2f"`) que `metrics_db03_db04.yaml` ya declara para `indice_riesgo_promedio`/`d1_promedio`/`d2_promedio` y lo pasa como `y_axis_format` — el YAML está bien. El defecto está en que ese `y_axis_format` no termina de forzarse hasta las etiquetas del eje en este tipo de chart, y en que no hay ninguna configuración de rotación/intervalo de etiquetas para el eje X cuando hay muchas categorías — la misma función compartida que ya produjo `BUG-064` en "Driver dominante" de DB-01. **Los datos son correctos** — es presentación, no un valor mal calculado. Detectado por **Oscar Antonio Quiroz Lázaro (C2)**, QA pre-demo en producción, sesión `analista`, 2026-09-07 ~21:25pm CST — ver [[vault/06_Quality_Testing/QA_Logs/2026-09-07-oscar-quiroz-qa-pre-demo]] casos 17-18. **Ampliado el mismo día, tercer tablero confirmado:** en DB-05 · Análisis por driver (tab D1), "% escuelas por driver dominante por ciclo" (`echarts_timeseries_line`) muestra el eje Y en "0,0,0,0,0" y **la etiqueta del propio punto de dato también lee "0"** (sin el signo %), cuando el valor real es 6.3% (visible en la tarjeta KPI-07 del mismo tab). Aquí el defecto ya no es solo el eje — ni siquiera la etiqueta del dato individual respeta el formato porcentual, así que `_params_chart()` no está forzando el `y_axis_format` de forma confiable en ningún punto de renderizado de `echarts_timeseries_line`, no solo en `_bar`. Mismo código compartido, tres tableros (DB-01, DB-04, DB-05), owners distintos (Manuel, Marina, Monserrat) — confirma que es un defecto de la función compartida, no de ningún YAML individual. **Acotado el mismo día — pista importante para quien lo corrija:** en DB-06 · Predicciones, "Variación proyectada (ML-01) por ciclo" (`echarts_timeseries_line`, misma métrica `variacion_proyectada_promedio`, **mismo `formato: porcentaje_1`** que `pct_escuelas_por_driver` de DB-05, que sí falla) renderiza el eje Y **correctamente** (0.0%, -0.2%, -0.4%... hasta -1.0%) y la etiqueta del punto de dato también correcta (-0.9%). Confirmado contra `metrics_db06_db09.yaml` y `metrics_db05_db08.yaml`: ambas métricas declaran exactamente el mismo `formato: porcentaje_1` — **el defecto no está en el string de formato ni es universal a todo `echarts_timeseries_*`**, así que la causa real no es la que se documentó originalmente (`y_axis_format` no forzado). Hipótesis más probable, sin verificar: el rango del eje se autoescala distinto según qué tan angosto es el rango real de los datos (el caso roto vive muy cerca de 0 sobre un dominio implícito 0-1; el caso sano tiene un rango de -1% a 0% bien distribuido) — mejor pista para depurar que la hipótesis original | **medium** | open | US-212 / US-213 / REQ-002 | **decidido 2026-09-08:** dueño C2 (código compartido de Célula 2, `superset/sync_semantic_layer.py`), **reviewer Manuel Serranía** (criterio BUG-037), fix post-9 (sync congelado). **No corregir corriendo `sync_semantic_layer.py` antes del 9-sep** (regla de ventana vigente) — cualquier fix espera a después de la demo | pendiente — ningún test cubre hoy el formato de eje Y ni la rotación/muestreo de etiquetas de eje X en los charts `echarts_timeseries_*` declarativos |
| BUG-067 | **DB-07 en producción: "Total de escuelas" y "Escuelas sin dato" inflados ×6 — el grano del cubo trae una fila por driver, y la métrica no deduplica.** `gold.cubo_completitud` (y el dataset `db07_cubo_completitud` que lo expone 1:1) tiene grano `cve_mun × nivel × id_driver × id_ciclo` (documentado en `dbt/models/gold/cubo_completitud.sql` y `metrics_db07.yaml`): **una fila por cada uno de los 6 drivers**, con `total_escuelas` repetido idéntico en las 6 filas de esa combinación municipio×nivel×ciclo. La métrica `total_escuelas` (`metrics_db07.yaml`) hace `SUM(total_escuelas)` directo sobre ese grano, así que cuenta cada escuela **6 veces**, una por driver. Verificado en vivo: "Total de escuelas" = 264,684 = exactamente `44,114 × 6` (44,114 es el total real de escuelas en alcance, confirmado contra DB-01/DB-04). "Escuelas sin dato" = 100,747 sufre el mismo mecanismo, con inflación variable según la cobertura de cada driver. **Es el mismo tipo de defecto que BUG-047/066 (grano multi-fila sumado sin deduplicar), pero en una dimensión distinta (driver, no ciclo) que ese fix nunca cubrió** — y aquí no hay ningún filtro de driver en el tablero que pudiera mitigarlo, es puramente el `SUM` de la métrica. **Los dos KPI de porcentaje (KPI-05 completitud 62%, KPI-06 % SIN_DATO 38.1%) son correctos**: al ser razones `SUM/SUM` sobre el mismo grano, numerador y denominador se inflan igual y el cociente no cambia — coincide exacto con la cifra de referencia de Diana (0.62). Por eso pasó desapercibido en verificaciones previas: solo se habían comparado los porcentajes, nunca los conteos absolutos contra el total real de 44,114. Detectado por **Oscar Antonio Quiroz Lázaro (C2)**, QA pre-demo en producción, sesión `analista`, 2026-09-07 ~22:05pm CST | **medium** | open | US-222 / REQ-002 | **Oscar Antonio Quiroz Lázaro (C2)** — es mi propio archivo (`metrics_db07.yaml`, US-222). Fix de fondo probablemente requiere un dataset separado ya deduplicado a grano municipio×nivel×ciclo para los tiles de conteo absoluto, no solo ajustar la expresión de la métrica actual. **No corregir corriendo `sync_semantic_layer.py` antes del 9-sep** (regla de ventana vigente) | pendiente — ningún test verifica hoy que los conteos absolutos de `db07_cubo_completitud` coincidan con el total real de escuelas en alcance, solo se prueban las razones |
| BUG-068 | **DB-09 en producción: "Escuelas a intervenir (mayor riesgo)" muestra escuelas sin predicción (N/A) antes que las de mayor riesgo real — Postgres ordena los `NULL` primero en `ORDER BY DESC`.** El chart "Escuelas a intervenir (mayor riesgo)" (`superset/dashboards/db09_recomendaciones.yaml`, dataset `db09_cubo_recomendaciones`, métrica `indice_riesgo`) usa `order_desc: True` (fijo para todo chart tipo `table` en `_params_chart()`, `superset/sync_semantic_layer.py`) sin especificar `NULLS LAST`. Postgres, por default, ordena `NULL` **primero** en un `ORDER BY ... DESC` — así que las escuelas sin `indice_riesgo` (comunitarias/multigrado, fuera de la cobertura de la muestra de ML-01, ver subtítulo de KPI-12 en DB-06: "cobertura de la muestra") aparecen en las primeras filas de una tabla que se llama literalmente "escuelas a intervenir", **antes** que las escuelas con el índice de riesgo más alto real (0.57, 0.52...). Verificado en vivo: las primeras ~6 filas visibles (Lagos de Moreno, Encarnación de Díaz, Ojuelos de Jalisco — todas escuelas "PREESCOLAR COMUNITARIO"/similares) muestran `N/A`, y solo después aparecen las filas con valor numérico real. Con `row_limit: 50`, si hay suficientes escuelas sin predicción, podrían desplazar por completo a las escuelas que sí deberían encabezar la lista de intervención. Detectado por **Oscar Antonio Quiroz Lázaro (C2)**, QA pre-demo en producción, sesión `analista`, 2026-09-07 ~22:15pm CST | **medium** | open | US-204 / REQ-002 / REQ-003 | Sin dueño asignado todavía — `superset/sync_semantic_layer.py` es código compartido de Célula 2; también podría corregirse a nivel del dataset (`db09_cubo_recomendaciones` con un `ORDER BY indice_riesgo DESC NULLS LAST`, o filtrando `cobertura_prediccion = 'OK'` antes de ordenar). **No corregir corriendo `sync_semantic_layer.py` antes del 9-sep** (regla de ventana vigente) | pendiente — ningún test cubre hoy el orden de tablas `table` con valores NULL en la métrica de orden |
| BUG-069 | **DB-10 en producción: "Fuentes con dato" = 11, cuando el catálogo declarado en el propio subtítulo tiene solo 8 fuentes (DS-01...DS-08).** `metrics_db10.yaml` (mi propio archivo, US-223) declara el grano de `db10_cubo_pipeline` como `[id_fuente, fecha_ingesta]` — **una fila por fuente y por fecha de ingesta**, no una fila por fuente. La métrica `fuentes_ok` hace `SUM(es_ok)` directo sobre ese grano: si una fuente se ingirió en más de una fecha distinta a lo largo del proyecto (varias se cargaron o recargaron en distintas sesiones), cada fecha suma "1" de forma independiente, así que el conteo de fuentes con dato supera el total real de 8. La tabla "Estado de las 8 fuentes" del mismo tablero confirma que solo existen 8 fuentes reales, todas `cobertura_pipeline = OK` — el 11 es matemáticamente imposible frente a su propio universo declarado. **Es el mismo tipo de defecto que `BUG-067`** (grano multi-fila sumado sin deduplicar), otra vez en un archivo propio (US-223, como `BUG-067` lo es de US-222). "Última ingesta" = ".409ms" es la limitación cosmética ya conocida y aceptada (documentada desde el cierre de US-223), no forma parte de este hallazgo. Detectado por **Oscar Antonio Quiroz Lázaro (C2)**, QA pre-demo en producción, sesión `analista`, 2026-09-07 ~22:22pm CST | **medium** | open | US-223 / REQ-002 | **Oscar Antonio Quiroz Lázaro (C2)** — es mi propio archivo (`metrics_db10.yaml`, US-223). Fix probable: `COUNT(DISTINCT id_fuente)` filtrado por `es_ok`, en vez de `SUM(es_ok)` — mismo patrón que Monserrat ya aplicó correctamente en DB-08 ("Escuelas en el explorador", `COUNT DISTINCT cct`). **No corregir corriendo `sync_semantic_layer.py` antes del 9-sep** (regla de ventana vigente) | pendiente — ningún test verifica hoy que "Fuentes con dato"/"Fuentes SIN_DATO" no excedan las 8 fuentes reales del catálogo |
| BUG-070 | **El refresco automático del access token está en `main` desde el 6-sep y NUNCA llegó a producción: la sesión se rompe a los 15 minutos, en silencio.** Reportado por **Karla Monter** probando en vivo: login 6:24 → a los 16 min el Panel ML rechaza la predicción (*"La API rechazó la solicitud"*); con login fresco y el mismo CCT funciona. **Su diagnóstico —que el frontend nunca llama a `/auth/refresh`— es correcto sobre lo desplegado y falso sobre el repo**, y esa diferencia es el bug: `src/frontend/auth.py:115` define `_refrescar()`, la 126 hace `POST /api/v1/auth/refresh` y la 148 refresca dentro del margen de 120 s; `2_Panel_ML.py:260,294` y `3_Chat.py:39` ya piden el token por `token_de_acceso()`. Entró con el PR #267 el **2026-09-06 01:38**. **Causa raíz: la imagen desplegada nunca se construyó desde `main`** — es la cadena `embed-combo-dec019` → `embed-combo-dec019-logout` (rev `faro-frontend-00008-fpw`), parchada a mano, y el propio DevLog de C5 dice *"el diff frente a la imagen desplegada es SOLO el logout"*. **Es `BUG-061` materializándose como falla de demo.** | **critical** | open | US-405, US-207, US-305, BUG-061, BUG-059, REQ-004, REQ-005 | **C5 (Luis Téllez): reconstruir la imagen del frontend desde `main`**, no parchar la anterior. Requiere antes que **C2 commitee el embebido** (`BUG-061`), o el rebuild pierde Dashboards. Es un solo trabajo: cerrar `BUG-061` cierra éste | **Riesgo directo sobre el 9-sep:** si la sesión se abre en la preparación y la demo empieza 15 min después, el Panel ML y el chat fallan a media presentación **sin decir "vuelve a iniciar sesión"**. Mitigación de sala: iniciar sesión **dentro** de la demo, no antes |
| BUG-071 | **Las tres páginas internas de FARO Web renderizan sin exigir sesión; dos lo hacen sin avisar.** Reportado por **Karla Monter** y **verificado de forma independiente por el PM con Playwright contra producción** (casos `WEB-Dashbo`, `WEB-Panel_`, `WEB-Chat`): **Dashboards** y **Chat** cargan sin un solo aviso; **Panel ML** sí avisa *"Inicia sesión"* pero deja usar el formulario. Confirmado en código: `grep -cE "st.stop\(\)|require_role|if not user"` da **0** en los tres archivos de `src/frontend/pages/`. **El agravante está en Dashboards**: `superset_client.py:51::_admin_token()` autentica con credenciales **admin propias** contra `/api/v1/security/login`, así que el embebido **no depende en absoluto de la sesión FARO** — sin login los 10 tableros quedan en blanco en silencio, sin explicación. | medium | open | US-206, US-207, US-305, US-405, REQ-002, REQ-004 | **C2 (Manuel Serranía / Marina García)**: una guarda de sesión al inicio de cada página —`if not current_user(): st.info(...); st.stop()`— reusando el `encabezado()` que Christian ya dejó en `auth.py:209`. **No es una fuga de datos**: la API rechaza con 401 y el 403 por rol funciona (verificado por Karla). Es de experiencia y de percepción | **Para el 9:** si el evaluador abre Dashboards antes de iniciar sesión ve 10 tableros en blanco y concluye que están rotos. Ésa es la razón por la que sube de cosmético a medium |
| BUG-072 | **DB-04 en producción: "% escuelas en riesgo" muestra "0.0%" cuando la medición real es 0.0155% — el d3format de la métrica con un solo decimal no puede representar valores tan pequeños.** `pct_escuelas_en_riesgo` (`SUM(escuelas_en_riesgo) / NULLIF(SUM(escuelas_con_prediccion), 0)`) declara `formato: porcentaje_1` = d3 `",.1%"` (mapeo en `FORMATO_D3`, `superset/sync_semantic_layer.py`); con el conteo real 7 de 45 276 (0.0155%) el tile big_number_total de DB-04 redondea a "0.0%" — el menor valor representable con 1 decimal de % es 0.1%, y esta razón vive tres órdenes abajo. El dato es correcto — es precisión de presentación, no de cálculo. Afecta además DB-02 ("% de escuelas en riesgo" y la fila del ranking, los 2 datasets de `metrics_db01_db02.yaml`) y DB-09 (`metrics_db06_db09.yaml`), por compartir la misma métrica. Detectado por **Diana Varela (C1)**, QA pre-demo en producción, sesión `ciudadano`, 2026-09-07 (plan de pruebas KPI-04) | **low** | open | US-203 / US-204 / US-212 / US-213 / REQ-002 | **decidido 2026-09-08:** dueño C2, **reviewer Manuel Serranía**. **Código preparado e inerte en la rama** (aplica solo con el re-sync): `FORMATO_D3` += `"porcentaje_3": ",.3%"` y `formato: porcentaje_3` en los 4 `pct_escuelas_en_riesgo` (`metrics_db01_db02.yaml` ×2, `metrics_db03_db04.yaml`, `metrics_db06_db09.yaml`) → "0.015%" visible. **No corregir corriendo `sync_semantic_layer.py` antes del 9-sep** (regla de ventana vigente); el re-sync físico entra en el bloque post-9 de BUG-064/066 | pendiente — se valida en el re-sync post-9: el tile DB-04 debe leer "0.015%" con el conteo 7/45 276; `tests/test_sync_formato_d3_cobertura.py` ya exige que todo `formato:` tenga clave en `FORMATO_D3` |
| BUG-073 | **Panel ML (US-207): los textos de ejemplo muestran CCTs ajenos al par de demostración.** `src/frontend/pages/2_Panel_ML.py`: `EJEMPLOS` (:59) y el `placeholder` del buscador (:268) muestran `15DJN0049A` / `09DSN0042A` (CCTs de la validación del camino del agente, no del guion); el par oficial de `US-006` es `15DPR0920D` / `15DPR2254O` (índice 0.4774 idéntico). Quien copie un ejemplo del buscador no reproduciría la ficha del guion de la demo — inconsistencia de texto, no de dato; no bloquea la demo de hoy (el guion escribe los CCT a mano). Detectado por **Diana Varela (C1)**, QA pre-demo, 2026-09-07 | **low** | fixed | US-006 / US-207 / US-526 / REQ-002 | **Corregido 2026-09-08 (Marina García del Buey, C2), con autorización explícita del PO.** `EJEMPLOS` y `placeholder` → `15DPR0920D` / `15DPR2254O`. **Se adelanta al post-demo porque la razón para aplazarlo desapareció**: se difirió por *"no tocar `src/frontend/**` por riesgo de regresar el embebido (BUG-061)"*, y el mismo PR que lleva este cambio es el que **commitea el embebido a `main`** y cierra BUG-061 — a partir de él un cambio en `src/frontend/**` ya no puede regresarlo | `tests/test_frontend_panel_ml_streamlit.py::test_los_ejemplos_son_el_par_oficial_de_la_demostracion` — **validada reintroduciendo el defecto** (con los CCT viejos reprueba). Mira solo líneas de código, no comentarios: la primera versión reprobaba por su propia documentación. Pendiente aún la revisión visual tras el deploy de C5 |
| BUG-074 | **La suite del embebido de Superset era intermitente por un defecto de la PRUEBA, no del código: el servidor falso habla HTTP/1.0 y cierra el socket tras cada respuesta.** `SupersetHTTPFake` hereda de `BaseHTTPRequestHandler`, cuyo `protocol_version` es `HTTP/1.0` por defecto, mientras `superset_client.tableros_embebidos()` reutiliza **un solo** `httpx.Client` para las ~22 peticiones del flujo (login, csrf, y slug + embed por cada uno de los 10 tableros): a partir de la segunda toma del pool encuentra la conexión cerrada. **En Linux la carrera se pierde solo a veces —de ahí la intermitencia—; en Windows falla siempre**, con `httpx.ReadError [WinError 10053]`. El síntoma engaña: como `1_Dashboards.py` captura `httpx.HTTPError` y lo pinta con `st.error`, no se ve una excepción sino *"no se montó ningún tablero"*, que parece un defecto del embebido. **No es `BUG-059`** —esa es la sesión de `auth.py`, cerrada por C4 el 6-sep— ni afecta a producción: el código de aplicación no cambia. Diagnosticado al aplicar el handoff de C5 (`BUG-061`), que reprobaba 2 de 2 antes de tocar nada; el mismo defecto existía en la versión previa de la prueba en `main`, así que no lo introdujo el handoff | low | fixed | US-206 / US-526 / BUG-061 | **Corregido 2026-09-08 (Marina García del Buey, C2).** `protocol_version = "HTTP/1.1"` en `SupersetHTTPFake`; `_respond` ya emitía `Content-Length`, así que la conexión se mantiene viva y el flujo completa. Sin cambio en lo que la prueba afirma | `tests/test_frontend_dashboards_streamlit.py` — verificado con **5 corridas consecutivas en verde** tras el fix, y reprobando de forma determinista antes |
| BUG-075 | **`FRONTEND_REDIRECT_URIS` nunca se pasaba al Cloud Run real: aunque el valor sea correcto, el login e2e en producción falla porque `/auth/login` rechaza el `redirect` del front con un 400.** El `--set-env-vars` de `vault/08_CICD_DevOps/scripts/deploy-cloud-run.sh` pasaba `GOOGLE_CLIENT_ID`, `GOOGLE_REDIRECT_URI` y `ANALISTA_EMAILS` (y los secretos por `--set-secrets`), pero **`FRONTEND_REDIRECT_URIS` no aparecía en ninguno de los dos**, así que el contenedor tomaba el default de desarrollo (`http://localhost:8501`, `src/api/config.py:59`). En producción el `redirect=window.location.origin` del React (`https://faro-frontend-eanzfglvyq-uc.a.run.app`) nunca coincide por **comparación exacta** (`config.py:130` split por coma + `auth.py:100` `not in`), así que `_validar_redirect` responde 400 y el login no completa. Es el gemelo en producción del hueco que Diana ya corrigió en `docker-compose.yml` para local. Detectado por **Diana Alvarez (C1)** al probar el botón de login (US-405) en vivo; reportado a C5/Christian por tocar `vault/08_CICD_DevOps/**` (regla 7) | high | fixed | US-405 / ADR-012 / REQ-004 | **Corregido 2026-09-12 (Luis Téllez, C5).** `deploy-cloud-run.sh`: se define `FRONTEND_REDIRECT_URIS` (default = origen canónico de FARO Web `https://faro-frontend-eanzfglvyq-uc.a.run.app`, sin barra final) y se agrega a `--set-env-vars` **antes** de `ANALISTA_EMAILS` (que queda al final por la coma del caso multi-correo), con comentario de la regla de coincidencia exacta y la receta multi-origen (alias con número de proyecto y delimitador alterno). **No toca producción**: el valor solo aplica cuando se corra el deploy. Credenciales reales de Google y el redeploy siguen pendientes (Luis, regla 7). Va en el PR de `dev/luis-tellez` que porta este commit | Sin test automatizado (script de deploy, no código de app); `bash -n` limpio. Validación e2e diferida al redeploy con credenciales reales: login real hasta la sesión FARO en la URL pública |
| BUG-076 | **El build del front no inyecta `VITE_API_ORIGIN`: la imagen se hornea con la variable vacía y el botón de login sale por el proxy del front, así que el `/callback` responde 401 aunque BUG-075 y las credenciales estén bien.** `frontend/src/lib/api.js:30` lee `import.meta.env.VITE_API_ORIGIN ?? ""` para construir la URL **absoluta** del login (`getAuthLoginUrl()`, US-405), que debe navegar **directo al origen de la API** —no por el proxy `proxy_pass` de nginx— porque la cookie anti-CSRF `faro_oauth_state` se fija en el origen que responde; si sale por el proxy, Google regresa al `/callback` del origen de la API, que nunca la fijó → 401 (documentado en `api.js:20-29` y en el hallazgo de Christian del PR #304 / `Threat_Model` v1.3). Pero **nada horneaba la variable en build-time**: `docker/frontend-react.Dockerfile` no declaraba `ARG`/`ENV VITE_API_ORIGIN`, `build-and-push-frontend.sh` no pasaba `--build-arg`, y no existe `frontend/.env.production` versionado ni un `define` en `vite.config.js` (verificado con `git ls-files`). Vite congela `import.meta.env.VITE_*` en compilación, así que con la variable vacía `getAuthLoginUrl()` cae a una ruta relativa que pasa por el proxy. Es el gemelo del lado cliente de BUG-075: BUG-075 hace que la API acepte el `redirect`; este hace que el front mande el login al origen correcto. Detectado por **Luis Téllez (C5)** al preparar el runbook del redeploy del front | high | fixed | US-405 / ADR-012 / REQ-004 | **Corregido 2026-09-12 (Luis Téllez, C5).** `docker/frontend-react.Dockerfile`: `ARG VITE_API_ORIGIN=""` + `ENV VITE_API_ORIGIN=$VITE_API_ORIGIN` antes de `RUN npm run build` (default vacío preserva el comportamiento de dev), y comentario corregido que distingue `VITE_API_BASE_URL` (datos, va por proxy, se queda vacío) de `VITE_API_ORIGIN` (login, se hornea, va directo). `build-and-push-frontend.sh`: define `VITE_API_ORIGIN` (default = origen de la API `https://faro-api-eanzfglvyq-uc.a.run.app`, sin barra final, el mismo valor de `FRONTEND_REDIRECT_URIS`) y lo pasa como `--build-arg`, con `echo` del valor. **No se tocó `frontend/src/lib/api.js`** (alcance de C1; ya lee la variable bien). **No toca producción**: solo aplica al reconstruir la imagen. Va en el mismo PR de `dev/luis-tellez` que BUG-075 | Sin test automatizado (Dockerfile + script de build, no código de app); `bash -n` de `build-and-push-frontend.sh` limpio. Validación e2e diferida al redeploy: el login cierra cuando el front reconstruido navega directo al origen de la API y la sesión FARO se fija en el origen del front |
| BUG-077 | **`GET /api/v1/municipios` (con o sin filtro) responde `500 internal_error` para el 97% de los municipios del universo: `MunicipioOut.nombre_entidad` es `StrictStr` (obligatorio, sin `None`), pero `gold.dim_municipio.nombre_entidad` es `NULL` en 307 de 317 filas.** `dim_municipio.sql` (dbt) toma `nombre_entidad` de `silver.rezago_municipio` (CONEVAL) por `LEFT JOIN`, bajo el supuesto documentado en el propio modelo de que *"CONEVAL publica el índice de rezago para TODOS los municipios del país, así que cada municipio del catálogo INEGI tiene su fila y su nombre_entidad"* — supuesto que no se cumple en el Postgres local verificado (solo ~2-3 municipios por entidad tienen `nombre_entidad`/`poblacion`, el resto `NULL`). El propio equipo ya había anticipado este modo de falla exacto: existe un `data_test` (`not_null_dim_municipio_nombre_entidad`, `_gold__models.yml`) diseñado para **fallar ruidosamente** si esto pasa — **y sí falla** (`dbt test --select gold`: 8 `ERROR`, dos de ellos `not_null_dim_municipio_nombre_entidad`/`_poblacion` con 307 resultados cada uno). **El hueco real no es el dato, es que nada corre esa prueba**: ningún workflow de `.github/workflows/*.yml` ejecuta `dbt test` ni `dbt run` — el único paso de CI es `dbt parse` (solo sintaxis). La guarda que el equipo construyó para esto nunca se ejecuta en ningún punto del pipeline. Además, aun si el dato estuviera completo, el código no tiene ninguna defensa: un `NULL` real (SIN_DATO explícito, como ya se hace con `poblacion`/`pobreza_pct` del mismo modelo) hoy revienta la serialización de Pydantic en vez de degradar. Detectado por **Edgar Coronel (PO)**, corrida de regresión local del 12-sep, verificado también contra `/api/v1/municipios/{cve_mun}` para municipios concretos (`14113`, `14112`...) y contra el listado sin filtro (primera página) | **critical** | fixed (frente de dato) | US-621 / REQ-002 / RISK-006 | **Frente (1) resuelto y verificado (12-sep, Diana Alvarez), va en el PR de `dev/diana-alvarez`:** `dbt test --select dim_municipio` -> 17/17 en verde, `GET /api/v1/municipios` -> 317/317 municipios con nombre_entidad, 0 nulos, sin ningun 500. `dim_municipio.sql` ya no depende del LEFT JOIN a CONEVAL para `nombre_entidad` -- se resuelve contra un catálogo propio (`dim_entidad`, seed nuevo) por `cve_ent`, así que deja de importar la cobertura real de CONEVAL en producción para este síntoma (branch `fix/bug-077-nombre-entidad-municipios`, ya reconciliado en `dev/diana-alvarez`, ver [[vault/_DevLog/2026-09-12-diana-alvarez-bug077-nombre-entidad|DevLog]]). Dos frentes independientes: (1) **Diana Alvarez (C1)**, dueña de `dbt/**` -- **hecho, ver nota arriba**; (2) **Christian Ruiz (C4)**, dueño de `src/api/**` — `nombre_entidad` debería declararse `StrictStr \| None` con el mismo criterio `SIN_DATO` que ya usa `poblacion`/`pobreza_pct` en el mismo esquema, para que un hueco de dato futuro degrade en vez de tronar con 500. **Urgente de verificar antes del *code freeze*** de mañana 20:00: si producción tiene el mismo hueco, cualquier pantalla que liste municipios está rota hoy | **Pendiente.** Ninguna prueba de contrato (`tests/test_api_contract.py`) cubre hoy un municipio con `nombre_entidad` nulo — todas usan el fixture de 4 municipios ya completos. Falta un caso que inserte un municipio con `nombre_entidad=None` y confirme degradación explícita, no 500. **Corrección 2026-09-13 (hallazgo real de revisión del PR):** el seed `dim_entidad` no tenía `+column_types` en `dbt_project.yml`, así que dbt infería `cve_ent` como `INTEGER` (pierde el cero a la izquierda, `"09"` -> `9`); `dim_municipio.sql` compara contra `substring(cve_mun,1,2)` (texto) -- el JOIN nunca coincide, Postgres revienta con `operator does not exist: integer = text` antes de construir el modelo. Los números 17/17 y 317/317 de la nota de arriba **no pueden venir de este código tal cual estaba committeado** -- ya se corrigió (`+column_types: {cve_ent: varchar(2)}`, mismo patrón que `dim_driver`), pero falta volver a correr `dbt seed --select dim_entidad --full-refresh && dbt run --select dim_municipio && dbt test --select dim_municipio` de verdad y pegar la salida real en el PR antes de dar el frente de dato por verificado otra vez |
| BUG-078 | **`deploy-cloud-run-frontend.sh` heredaba `--no-cpu-throttling` del servicio Streamlit previo, así que Cloud Run rechazaba el deploy del front React con `256Mi`** (`Total memory < 512 Mi is not supported with cpu always allocated`). nginx que sirve el SPA es request-driven y no necesita CPU siempre asignada, pero el script no lo declaraba y el servicio conservaba la CPU siempre asignada del deploy anterior. Detectado por **Luis Téllez (C5)** al desplegar el front React a Cloud Run. | medium | fixed | US-405 / REQ-005 / ADR-012 | **Corregido 2026-09-13 (Luis Téllez, C5).** `deploy-cloud-run-frontend.sh`: se fija `--cpu-throttling` (CPU solo durante requests → `256Mi` basta) y se reescribe el comentario del encabezado, que aún describía el modelo CORS/`VITE_API_BASE_URL` anterior a Camino B. Va en el PR de `dev/luis-tellez` que porta este commit. | Sin test automatizado (script de deploy, no código de app); `bash -n` limpio. Validado en vivo: el deploy del front completa y la revisión sirve al 100% (rev `faro-frontend-00009-way`). |
| BUG-079 | **Login e2e roto en prod: la imagen de la API desplegada (`agente-457715a`, 8-sep) era anterior a la sesión por cookie de ADR-012 (PR #304), así que nunca sembraba `faro_sesion` y `/auth/me` daba `401` permanente.** La imagen no contenía `src/api/security/cookies.py`, su `POST /auth/exchange` ignoraba `?sesion=cookie` (solo devolvía `TokenPair` JSON) y `get_current_user` leía solo el encabezado `Bearer`, nunca la cookie. Con `SEC-006` (`AUTH_LECTURA_PUBLICA=false`) el efecto es que —aun pasando el login de Google y con `BUG-075`/`BUG-076` ya resueltos— el front (`react-d10ce60`) canjeaba el `code_faro` pero la API no sembraba la sesión, así que el `SessionProvider` quedaba en `anonimo` y las cifras daban `401`. Front y API **desalineados**. Detectado por **Luis Téllez (C5)** al validar el login e2e en vivo. | **critical** | fixed | US-405 / ADR-012 / ADR-010 / REQ-004 | **Resuelto 2026-09-13 (Luis Téllez, C5) — operacional, sin cambio de código.** Reconstruí la imagen de la API desde `main` (`faro-api:agente-6896255`, `--platform linux/amd64`), que sí trae la sesión por cookie y las deps del agente, y la desplegué **solo al contenedor `api`** (`--container=api`) con deploy pin-first: clavé el tráfico a la revisión sana (`faro-api-00019-2xs`), la nueva `faro-api-00020-nwg` nació al 0%, la verifiqué en la URL de preview y moví el 100% (`--to-latest`). El swap de un contenedor preserva el sidecar ChromaDB, `SEC-006` y las env vars/secretos. Rollback: `update-traffic --to-revisions=faro-api-00019-2xs=100`. Detalle en el DevLog `2026-09-13-luis-tellez-login-e2e-redeploy-api`. | **Verificado e2e en vivo por Luis (2026-09-13):** login de Google completo → nombre visible arriba y cifras de la home cargan. En preview: `/version=6896255`, `SEC-006 /kpis=401`, `exchange?sesion=cookie` con code inválido → `401` limpio (no 500), revisión `Ready` con sidecar `ContainerHealthy`. Sin test automatizado (redeploy operacional). |
| BUG-080 | **`build-and-push.sh` no fija `--platform linux/amd64`: en un host arm64 (Docker Desktop en Mac Apple Silicon) construye una imagen arm64 que Cloud Run (amd64) no puede ejecutar** (`exec format error`). Defecto latente del script de build de la imagen de la API; salió a la luz al reconstruir la API para `BUG-079` desde una Mac Apple Silicon, donde hubo que añadir el flag a mano. Detectado por **Luis Téllez (C5)**. | medium | fixed | REQ-005 / ADR-012 | **Corregido 2026-09-13 (Luis Téllez, C5).** `build-and-push.sh`: `docker build` ahora pasa `--platform linux/amd64` (en CI/hosts amd64 es no-op), con comentario que explica el `exec format error` en Cloud Run. Va en el PR de `dev/luis-tellez` que porta este commit. | Sin test automatizado (script de build, no código de app); `bash -n` limpio. Validado en vivo: la imagen `faro-api:agente-6896255` construida con el flag corre en Cloud Run (rev `faro-api-00020-nwg`, `ContainerHealthy`). |
| BUG-081 | **La imagen de la API horneaba el modelo de embeddings equivocado, así que el chat respondía «El contexto de FARO no está disponible temporalmente».** La imagen cacheaba `all-MiniLM-L6-v2`, pero el agente (`src/agente/recuperacion.py`, `NOMBRE_MODELO_EMBEDDINGS`) y la colección de ChromaDB (`indexar_esquema.py`) usan `paraphrase-multilingual-MiniLM-L12-v2`. Como el runtime de la API corre OFFLINE (`HF_HUB_OFFLINE`), solo puede cargar un modelo ya cacheado en la imagen; al pedir uno ausente, la carga fallaba → `ErrorRecuperacion` → el endpoint del chat degradaba a «contexto no disponible». Los tests no lo cazaban porque no ejercitan la imagen de producción. Detectado por **Luis Téllez (C5)** al validar el chat en vivo el 2026-09-14. | high | fixed | US-304 / US-305 / REQ-006 | **Corregido 2026-09-14 (Luis Téllez, C5).** `docker/api.Dockerfile`: la línea que hornea el modelo pasa a `paraphrase-multilingual-MiniLM-L12-v2` + un comentario CRÍTICO que ata el nombre al default del código para que no vuelva a divergir. Imagen reconstruida en Cloud Build y desplegada solo al contenedor `api` (revisión `faro-api-00033-fim`, imagen `main-a8b8f252-hf1`), preservando el sidecar ChromaDB y `SEC-006`. Rollback a la revisión previa `faro-api-00031-gom`. Va en el PR de `dev/luis-tellez`. | Verificado e2e en vivo por Luis (2026-09-14): el chat responde de nuevo (un ranking de riesgo en lenguaje natural devolvió 10 escuelas con datos) y el guardrail sigue bloqueando lo destructivo (una orden de borrar una tabla se rechazó). Sin test automatizado (hotfix de imagen, no código de app); el nombre del modelo queda documentado en el Dockerfile |
| BUG-082 | **`GET /api/v1/about/secciones/modelo-datos` respondía 500 porque los GeoJSON que lee viven fuera de `src/` y no se copiaban a la imagen.** La sección «Cómo funciona / Modelo de datos» (US-601, `src/api/v1/about.py`) lee dos archivos de disco (`superset/assets/geojson/municipios_scope.geojson` y `mexico_silueta.geojson`); el `COPY src/` del Dockerfile no los incluía → `FileNotFoundError` → 500, y la pantalla «Cómo funciona» del front reventaba. Detectado por **Luis Téllez (C5)** el 2026-09-14. | high | fixed | US-601 / REQ-004 | **Corregido 2026-09-14 (Luis Téllez, C5).** `docker/api.Dockerfile`: se agrega `COPY superset/assets/geojson/` a la imagen. Desplegado en la misma revisión que BUG-081 (`faro-api-00033-fim`). Los `.dockerignore`/`.gcloudignore` usados para acotar el contexto del build (el repo pesa ~4.5 GB por entornos locales que ninguna imagen necesita) quedan **fuera de este PR**: son archivos de raíz sin dueño en `ownership.yml` (mismo hueco que `.gitignore`/`.gitattributes`, BUG-039) → follow-up con el PM para agregarlos a `comunes`. El fix versionado es `docker/api.Dockerfile`, en el PR de `dev/luis-tellez`. | Verificado e2e en vivo por Luis (2026-09-14): la pantalla «Cómo funciona» carga. Sin test automatizado (hotfix de imagen) |
...
## BUG-048 — `sources.yml` seguía sirviendo el fixture de SESNSP pese a existir el cargador real

Detectado por **Diana Alvarez (C1)** el 2026-09-05, verificando su propia sospecha de que el hueco
de completitud de D2 (inseguridad, SESNSP) caía en el alcance de Célula 1, antes de avisarle a
nadie. `src/ingesta/cargar_bronze_sesnsp_real.py` ya existía (Luis García, un día antes), pero
`dbt/models/sources.yml` seguía con:

```
identifier: "{{ var('bronze_sesnsp_identifier', 'sesnsp_test') }}"
```

Cualquier ambiente que corriera `dbt run` sin pasar `--vars '{bronze_sesnsp_identifier: sesnsp}'`
explícitamente seguía leyendo el fixture. **Fix:** default corregido a `'sesnsp'` — commit
`165c911` en `dev/diana-alvarez`. Verificado real contra Postgres, no solo por lectura de código:
`dbt run` (sin `--vars`) deja `silver.delitos_municipio` cruzando **2,486 municipios** reales;
`comp_media` de D2 se mantiene estable (0.4861 / 0.4827 / 0.4804 por ciclo) — el fix corrige la
fuente sin romper el cálculo existente. Avisado a Luis García (dueño de DS-04).

**Por qué queda `fixed (parcial)` y no `fixed`:** el fix resuelve por completo el lado de SESNSP,
pero D2 (y D1) siguen usando la población municipal de CONAPO como denominador/insumo, y esa
fuente (DS-08) sigue en fixture (`bronze.conapo_sample`, 36 filas / 12 municipios) — bloqueado en
que Emilio Galnares (dueño de DS-08) comparta el extractor real. Se investigaron fuentes públicas
alternas por si Célula 1 podía resolverlo de forma independiente dado el freeze del 6-sep: dos
candidatos descartados con evidencia real (certificado SSL expirado en un mirror comunitario;
otro mirror del propio ATDT resultó ser el producto de "Conciliación demográfica", solo a nivel
estatal, grano equivocado para el join municipal) y uno aceptado como opción viable pendiente de
decisión (`github.com/lapanquecita/poblacion-estimada`, mismo autor que el mirror de respaldo ya
documentado para SESNSP, MIT, SSL válido) — no adoptado hoy, para no sustituir en silencio el
pipeline que le pertenece a Emilio. Ver DevLog 2026-09-05 para el detalle completo, incluido un
hallazgo abierto no relacionado a este bug puntual: `gold.cubo_pipeline` reporta DS-08 como
`cobertura_pipeline = 'OK'` aunque sea fixture, porque el `_source` del fixture coincide con el
que usaría una carga real — pendiente de discutir con Deni Garrido y Edgar Coronel, no corregido
en este fix.

**Actualización 2026-09-05, más tarde el mismo día — CONAPO también cerrado:** Emilio Galnares
compartió por Teams el Parquet real de DS-08 (252,450 filas, 2,475 municipios, 32 entidades,
1990-2040). Verificado antes de programar nada: las 18 columnas de grupo de edad suman exacto
`POB_TOTAL` en las 252,450 filas (0 discrepancias), y el grano real es municipio × sexo × año
(2,475 × 2 × 51 = 252,450). Se construyó `src/ingesta/cargar_bronze_conapo_real.py`, que agrega
sexo con `sum()` **en el loader, no en Silver** — el dedup de `poblacion_municipio.sql`
(`row_number()` por `cve_mun, anio, grupo_edad` sin sumar) habría descartado de forma no
determinista la mitad de la población si HOMBRES/MUJERES llegaran como filas separadas con el
mismo `_ingested_at` — y normaliza `_source` a `'DS-08_CONAPO'` para que coincida con lo que
`cubo_pipeline.sql` espera. Default de `sources.yml` corregido a `'conapo'` (commit `19fecc4`).
**Verificado real contra Postgres:** `comp_media` de D2 sube de ~0.48 a ~0.62 y `d2_sin_dato` cae
de ~36,700 a **0** en los 3 ciclos — sin ningún caso restante de D2 sin dato por CONAPO. Con esto
**BUG-048 queda cerrado del lado de C1** (SESNSP y CONAPO, los dos denominadores/insumos de D2);
la fuente alterna de GitHub documentada arriba deja de ser necesaria y no se adopta. La entrada
de `cubo_pipeline` reportando DS-08 como `'OK'` con fixture (arriba) también queda resuelta de
facto: DS-08 ya no es fixture.

**Consolidación tras el merge de `origin/main` (mismo día):** esta alta compartía el ID BUG-048
con una alta previa del PM ("Producción sirve un Gold empobrecido", 2026-09-04) sobre la que C3
(Andrés González Habib) ya había registrado un rerun de ML contra el dump de C1. Al sincronizar
`dev/diana-alvarez` con `main`, la tabla de arriba fusiona ambas filas en una sola para no dejar
dos altas activas con el mismo ID describiendo el mismo bug — mismo criterio que usó Oscar Quiroz
al retractar su BUG-050 duplicado de BUG-047 (DEC-013). El estado combinado: **C1 fixed** (este
detalle), **C3 fixed** (rerun de ML), **C5 pendiente** (importar a Cloud SQL y validar Superset)
— el bug como un todo no cierra hasta que producción sirva el Gold real.
