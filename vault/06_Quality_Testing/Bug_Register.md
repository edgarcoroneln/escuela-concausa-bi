...
| BUG-064 | **DB-01 en producción: leyenda de pastel paginada/truncada y eje X del driver dominante sin etiquetas.** Tres síntomas del mismo tablero, agrupados por ser todos de legibilidad, no de dato: **(1)** "Escuelas por nivel (KPI-08)" (pastel, 3 categorías) muestra la leyenda **paginada** — solo 1 de 3 categorías visible (`◀ 1/3 ▶`), hay que darle clic para ver el resto; **(2)** el mismo pastel y "Escuelas por sostenimiento (KPI-09)" truncan las etiquetas de rebanada ("SECU...", "PR...", "PRI...", "PÚB...") — en el primero, "PR..." es **ambiguo** entre "Primaria" y "Preescolar", ambas empiezan igual; **(3)** "Driver dominante (KPI-07)" (barras, 6 drivers) solo muestra el nombre de la categoría bajo la primera barra ("Agua"); las otras 5 barras (valores 419, 12,765, 2,086, 2,792, 26,046, 6) no traen su nombre de driver en el eje X — es el tablero que muestra el diferenciador del proyecto y hoy nadie puede saber qué representa cada barra sin el eje. **Los datos son correctos en los tres casos** — es presentación, no un valor mal calculado. Detectado por **Oscar Antonio Quiroz Lázaro (C2)**, QA pre-demo en producción, sesión `analista`, 2026-09-07 ~20:43-20:44 CST — ver [[vault/06_Quality_Testing/QA_Logs/2026-09-07-oscar-quiroz-qa-pre-demo]] casos 3-5. **Ampliado el mismo día:** el mismo patrón de leyenda paginada (solo 1 categoría visible de varias, con `◀ N/M ▶`) se repite en DB-06 · Predicciones, "Distribución del riesgo proyectado" (pastel/donut) — cuarto síntoma del mismo defecto de legibilidad, ahora confirmado en dos tableros (DB-01, DB-06) | **medium** | open | US-203 / REQ-002 | Sin dueño asignado todavía — `superset/dashboards/db01_ejecutivo.yaml` es alcance verde compartido de Célula 2 (Manuel Serranía, US-203). **No corregir corriendo `sync_semantic_layer.py` antes del 9-sep** (regla de ventana vigente: el sync puede borrar el parche de 20 charts que Luis aplicó a mano, PR #275) — cualquier fix debe esperar a después de la demo o coordinarse explícitamente con el PO | pendiente — ningún test cubre hoy legibilidad de leyendas/ejes en los charts declarativos de `superset/dashboards/*.yaml` |
