...
<<<<<<< HEAD
| BUG-054 | **La tarjeta "% escuelas con recomendación" de DB-09 muestra `No data`.** Es la tarjeta de cobertura del tablero de **recomendaciones prescriptivas** — la narrativa central del proyecto—, así que se ve mal justo donde peor conviene. **El modelo y los datos NO son la causa**, verificado por el PM contra el Gold local: `gold.cubo_recomendaciones` tiene 55 filas para 2024-2025, las **55 con `recomendacion_emitida = 1`** sobre **55 CCT distintos**, así que la métrica `SUM(recomendacion_emitida) / NULLIF(COUNT(DISTINCT cct), 0)` (`metrics_db06_db09.yaml:222`) calcularía **100 %**, no `No data`. La columna existe en el modelo (`cubo_recomendaciones.sql:42`) y la métrica está declarada una sola vez, en el dataset `db09_cubo_recomendaciones`. **Sospechoso principal: `BUG-037`** — `sync_semantic_layer.py` actualiza el SQL del dataset pero nunca le pide a Superset que refresque el esquema, así que una métrica que referencia una columna que el dataset no conoce no se puede calcular; Marina García lo ha reportado **tres veces** y sigue `open`. Segunda hipótesis: el cubo sin filas para el ciclo seleccionado en ese ambiente. Reportado por Marina García (C2) el 2026-09-05 al cerrar `US-215a`, fuera de su historia | **high** | open | US-204 / US-214b / REQ-002 | pendiente (**C2 — Manuel Serranía**, dueño de DB-09 por `US-204`). Camino más corto: `PUT /api/v1/dataset/<id>/refresh` sobre `db09_cubo_recomendaciones` y reconfirmar la tarjeta — es la misma mitigación manual que Marina aplicó las tres veces que le pegó `BUG-037` | ⬜ **Verificar antes del 9-sep.** Una tarjeta en `No data` en el tablero prescriptivo es de lo más visible que puede encontrar el evaluador. Si no alcanza a arreglarse, hay que decidir si se retira la tarjeta del tablero o se explica en la narrativa — pero no dejarla así sin decisión |
=======
