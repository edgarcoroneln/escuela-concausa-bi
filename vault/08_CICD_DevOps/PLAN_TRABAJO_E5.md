...
| Docker build | **Validado 11-sep, contenedor non-root incluido.** `docker build` limpio (fix: `npm ci --legacy-peer-deps`). El contenedor `nginxinc/nginx-unprivileged` (pedido de Christian) se caía al arrancar (`open() /run/nginx.pid failed: Permission denied` -- la plantilla propia de nginx pisaba el ajuste de la imagen base); corregido moviendo `pid`/`*_temp_path` a `/tmp` en `docker/nginx-frontend.conf.template`. Verificado con `docker run` + `docker exec whoami` -> `nginx` + `curl` -> `200 OK` | Diana/Luis | Cerrado -- falta push a Artifact Registry |
...
| Mocks en recorrido productivo | **En progreso 10/11-sep (revisión de Edgar, PR #302).** Se agregó modo demo explícito (`VITE_USE_MOCK`, nunca fallback silencioso por error) + `getEscuelasEnRiesgo()` real (orden desc por `indice_riesgo`, sin `indice_riesgo_min` que el API no admite). `Home.jsx`, `LosSieteCasos.jsx` y `ExpedienteEscuela.jsx` ya corren 100% contra el API real, verificado en navegador (Network tab, respuestas reales). **BUG-064 encontrado y corregido 11-sep:** `getEscuelasEnRiesgo()` devolvía el sobre de paginación completo (`Page[EscuelaOut]`: `{items, total, page, size}`) sin desenvolver `items`; `LosSieteCasos.jsx` esperaba un arreglo, así que `escuelas.length` de un objeto salía `undefined` y la página quedaba en blanco sin ningún error visible. Corregido en `frontend/src/lib/api.js` desenvolviendo `data.items` dentro de `getEscuelasEnRiesgo()`. Bloqueante para conectar el resto: `EscuelaOut` no trae lat/lon ni variación por escuela; no hay endpoint de serie histórica ni de matriz de drivers en lote — ver §9 de `Arquitectura_Frontend_React`. **Decidido 11-sep:** `MapaCasos`, `MatrizDrivers`, `ComparacionTerritorial` y `Comparativa` muestran `EnConstruccion` (gap explicado por pantalla) en vez de mock sin rotular; `VistaGeneral` se conectó parcialmente — el bloque "El diferenciador" (par `15DPR0920D`/`15DPR2254O` de `Guion_Demo_US006`) ya usa `getEscuela`+`getPrediccion` reales para esas dos escuelas | Diana/E5 (implementación) + dueño de datos/API (el gap de contrato) | Resto de pantallas antes del viernes |
