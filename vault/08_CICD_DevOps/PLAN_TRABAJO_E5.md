...
| `prioridad`/`BUG-063` | ALTA usa `ANCLA_SIGMOIDE` (0.60) contra un máximo real de 0.5717 — ninguna de las 45,276 filas de `gold.recomendaciones` alcanza ALTA. Ya registrado en `main` (Héctor Morales, 7-sep) con la decisión explícita de posponerlo a post-demo. Hoy es post-demo: hay que empujar la decisión (0.60 vs. `LINEA_DE_ALERTA` 0.50) | Héctor Morales + Andrés González (C3), con el PO | Hoy — lo pidió Marina, afecta la tarjeta de prioridad de E3 |
