...
| BUG-017 | `indice_riesgo` se publicó **saturado**: la corrida real de ML-01 dio MAE 10.90, pero la sigmoide está calibrada sobre **fracción** (`-0.05` = pierde 5 % de matrícula). Con esa escala el 100 % de las 45 249 filas queda en riesgo ≈ 1.00 y el tablero cuenta como "en riesgo" a todo el universo. **Confirmado por Diana el 2026-08-28**: `target_variacion_matricula = matricula_total - matricula_ciclo_anterior`, diferencia absoluta de alumnos. El MAE 10.90 son ~11 alumnos, no un modelo malo; lo que está mal es publicar eso a través de una sigmoide calibrada sobre fracción. Añadida guarda que detiene la publicación en vez de saturar en silencio; **falta confirmar las unidades en Gold con C1** | high | **fixed** | US-311 / US-313 / REQ-003 / US-104 | **Resuelto por la ratificación de [[03_Architecture/ADRs/ADR-007-unidad-target-variacion-matricula\|ADR-007]] el 30-ago (DEC-012):** fracción, sin cambios a la sigmoide ni al umbral 0.6. La saturación desaparece porque la unidad del target coincide con la calibración. **Pendiente de implementación con dueño y fecha (R-4): reentrenar ML-01 — Héctor Morales.** C2 verifica DB-03 con predicciones nuevas publicadas, no con el ADR firmado | **La guarda `verificar_escala_variacion()` queda como control permanente, no medida temporal** (R-4 del ADR): es lo único que impidió publicar 45,249 filas saturadas en silencio |
| BUG-018 | ML-02 arrastra el **mismo defecto por ventana** que BUG-015: `entrenar_ml02._matriz()` toma siempre los 6 drivers sin comprobar cobertura dentro de la ventana de entrenamiento, así que un driver vacío en ese tramo (D6 aire, IDW de US-105) rompe el binning de `HistGradientBoostingClassifier` con el mismo error. Reproducido; el arreglo es el mismo que ya se aplicó en ML-01 | high | **fixed** | US-302 / REQ-003 | `feat/andres-habib-bug018-ml02-cobertura` (Andrés González) — cobertura evaluada por ventana y predicción/SHAP alineados con `feature_names_in_`. **Corregido en el registro el 30-ago**: la matriz de trazabilidad lo daba por resuelto desde el 28-ago pero esta fila seguía en `open`; el registro es la fuente canónica y no puede ir detrás de la matriz | 16 pruebas de ML-02 ✅ · ruff ✅ |
| BUG-019 | `target_variacion_matricula` se produce en **dos unidades distintas** bajo el mismo nombre: `features_escuela.sql` (C1, grano escuela) da **alumnos absolutos** y `target_hibrido.variacion_desde_serie` (C3, grano municipio×nivel) da **fracción**. Ambas llegan a `gold.predicciones.valor`, distinguidas sólo por `grano` (DEC-010), así que esa columna hoy mezcla alumnos con fracciones. El contrato nunca declaró la unidad | high | **fixed** | US-104 / US-311 / US-313 / REQ-003 | **Resuelto por la ratificación de ADR-007 el 30-ago (DEC-012).** El ADR amplió el alcance más allá del target de ML: **`fact_escuela_ciclo.variacion_matricula` tenía el mismo defecto y la leen cuatro tableros** — ratificar sólo el target dejaba el ML coherente y el frontend roto. **Dueño de la implementación: por asignar en el standup del 30-ago**, porque DB-06 y DB-09 de Manuel también leen `gold.predicciones` | La unidad se declara en `Data_Model.md` §5.3, no en el acuerdo (R-2). Convención nueva (R-3): toda columna que exprese una razón lleva `_pct`/`_frac`, o se guarda como numerador y denominador por separado |
| BUG-020 | En la URL pública **toda ruta que toca base de datos responde HTTP 500**: `/api/v1/predicciones/{cct}`, `/predicciones/batch` y `/escuelas`. `/api/v1/health` responde 200, así que el contenedor corre y el despliegue de BUG-008 sirvió. Con token válido, inválido o sin token el resultado es el mismo 500 —nunca 401—, así que el fallo ocurre **antes** de validar auth. Sin esto no hay demo end-to-end ni el punto de rúbrica de URL pública | **critical** | **fixed** | US-401 / US-411 / US-501 / REQ-004 / REQ-005 | **Luis Téllez (C5), 30-ago** — Fase 2: Cloud Run redesplegado con `--vpc-connector=faro-connector`, service account de mínimo privilegio, variables de conexión y secretos de Secret Manager. **Causa raíz:** el despliegue sólo pasaba `ENVIRONMENT=production`; sin variables de DB, `config.py` caía a su default `postgres_host="localhost"` y la API buscaba un Postgres inexistente dentro del contenedor. Eso explicaba los dos síntomas a la vez — `/health` en 200 por no tocar la base, y 500 en todo lo demás **antes** de evaluar auth, por eso nunca aparecía un 401 | `08_CICD_DevOps/scripts/smoke-test-bug020.sh` — **verificado en vivo el 30-ago: 4/4 etapas, exit code 0.** health 200 · `/escuelas` 200 con 25 CCT reales · ruta protegida responde 404 para CCT inexistente. Línea base previa: exit code 2 |
...
<<<<<<< Updated upstream
| BUG-025 | El endpoint desplegado `/api/v1/agente/consulta` es el **stub** de `src/api/v1/agente.py`: responde **la misma cadena fija a cualquier pregunta**, incluidas las fuera de alcance y las destructivas. Además su filtro de palabras busca `"borrar"` por subcadena, así que **«Borra la tabla de predicciones» no lo dispara** y recibe la respuesta normal con `fuera_de_alcance: false`. Los guardarraíles reales de `src/agente/guardrails.py` —que sí rechazan esa frase— nunca se invocan desde la API | high | **parcial** | US-304a / US-305 / REQ-006 | **El PR #142 (Christian Ruiz, C4) entrega el *seam*, no el agente.** El endpoint ya no responde una cadena fija: conecta con `src/agente/servicio.py` y los guardarraíles reales sí se invocan, así que **el SQL destructivo se rechaza**. Falta la generación real, con dueño: **(a) C3 · Andrés** — `generar_sql()` y `redactar_respuesta()` con LLM; **(b) C4 · Christian** — `ejecutar_sql()` read-only sobre Gold con rol de solo `SELECT`, `SET TRANSACTION READ ONLY` y `statement_timeout`; **(c) C5 · Luis** — deps del agente en la imagen de la API, ChromaDB operativo en el despliegue e índice del esquema Gold cargado. **Bloqueado por BLOCK-003**: no hay proveedor ni modelo de LLM acordado | `tests/test_agente_endpoint.py` verifica el rechazo de SQL destructivo. **Falta el E2E sobre la URL pública**: pregunta real → SQL generado → filas → respuesta redactada |
=======
| BUG-025 | El endpoint desplegado `/api/v1/agente/consulta` es el **stub** de `src/api/v1/agente.py`: responde **la misma cadena fija a cualquier pregunta**, incluidas las fuera de alcance y las destructivas. Además su filtro de palabras busca `"borrar"` por subcadena, así que **«Borra la tabla de predicciones» no lo dispara** y recibe la respuesta normal con `fuera_de_alcance: false`. Los guardarraíles reales de `src/agente/guardrails.py` —que sí rechazan esa frase— nunca se invocan desde la API | high | **parcial** | US-304a / US-305 / REQ-006 | **PR #142 (Christian Ruiz, C4) entrega el *seam*, no el agente.** El endpoint ya no responde una cadena fija: conecta con `src/agente/servicio.py` y los guardarraíles reales sí se invocan. **Lo que falta para el 100 %, con dueño:** (a) **C3 · Andrés** — `generar_sql()` y `redactar_respuesta()` con LLM real; (b) **C4 · Christian** — `ejecutar_sql()` read-only sobre Gold con rol de solo `SELECT` y `statement_timeout`; (c) **C5 · Luis** — deps del agente en la imagen de la API, ChromaDB operativo en el despliegue e índice del esquema Gold cargado. **Bloqueado por una decisión pendiente: proveedor y modelo de LLM** | `tests/test_agente_endpoint.py` — verifica que el SQL destructivo se rechaza en la capa SQL. **Falta el E2E sobre la URL pública**: pregunta real → SQL generado → filas → respuesta redactada |
>>>>>>> Stashed changes
