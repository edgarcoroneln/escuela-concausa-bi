...
| BUG-025 | El endpoint desplegado `/api/v1/agente/consulta` es el **stub** de `src/api/v1/agente.py`: responde **la misma cadena fija a cualquier pregunta**, incluidas las fuera de alcance y las destructivas. Además su filtro de palabras busca `"borrar"` por subcadena, así que **«Borra la tabla de predicciones» no lo dispara** y recibe la respuesta normal con `fuera_de_alcance: false`. Los guardarraíles reales de `src/agente/guardrails.py` —que sí rechazan esa frase— nunca se invocan desde la API | high | **fixed** | US-304a, US-305, REQ-006 | **PR #142** (Christian Imanol Ruiz Hurtado, C4): `/agente/consulta` deja de ser stub y llama al servicio RAG real con un seam de inyección de dependencias · **PR #148** (Luis Téllez, C5): el redeploy que lo lleva a producción — el código en `main` no cambiaba nada en prod, que seguía sirviendo la imagen `v0.2.1-hotfix-bug008` | Verificación en vivo del PM el 30-ago contra la URL pública: «cuál es la capital de Francia» devuelve `fuera_de_alcance: true`, ya no la cadena fija. **Ver BUG-033**: el agente responde, pero ChromaDB no está desplegado | 
...
| BUG-032 | `Data_Model.md` se contradice a sí mismo sobre dónde vive `indice_riesgo`: la línea 181 (§4.5) describe correctamente `valor` (variación cruda) e `indice_riesgo` como **columnas distintas** —que es lo implementado y lo que consume la API—, pero la nota de la línea 313 afirma que `indice_riesgo` vive *"en la columna `valor`"*. Quien lea §5.3 consultaría `valor` esperando un `[0,1]` y recibiría la variación cruda, hoy en alumnos absolutos: números como `-20` donde espera `0.6` | medium | open | US-313 / US-411 / REQ-003 | pendiente — **Diana Alvarez (C1)** | ver detalle |
| BUG-033 | **El agente no puede responder ninguna pregunta útil en producción.** BUG-025 quedó cerrado —el guardarraíl real opera y una pregunta fuera de tema se rechaza— pero de las **cuatro colaboraciones** que el servicio necesita, **ninguna está conectada**: `recuperar_contexto` falla porque ChromaDB no está desplegado, y `generar_sql`, `ejecutar_sql` y `redactar_respuesta` levantan `AgenteNoConfigurado` por defecto en `src/api/v1/agente.py`. **No existe ningún cliente LLM en el repositorio ni declarado en ningún `requirements`**, así que el Text-to-SQL —el núcleo del agente— está sin escribir. Hoy toda pregunta en alcance devuelve «El contexto de FARO no está disponible temporalmente» | **high** | open | US-304b, US-305, REQ-006 / REQ-005 | pendiente, en este orden: **(1) C5** desplegar ChromaDB e indexar el esquema · **(2) C3** implementar `generar_sql` y `redactar_respuesta` sobre el seam, con su dependencia LLM declarada · **(3) C4/US-404 + C5** ejecutor read-only sobre Gold con rol de solo lectura en Cloud SQL | — |
...
> **Cerrado el 2026-08-30.** Luis Téllez lo curó en producción en dos fases: US-504 aprovisionó la base
> privada (VPC, Cloud SQL, Secret Manager) y US-505 pobló Gold en Cloud SQL y redesplegó el API con
> conector VPC. **Verificado en vivo por el PM contra la URL pública**, sin fiarse del reporte:
>
> ```
...
> ```
>
> El `404` de `/predicciones/{cct}` **no es este bug**: la base conecta y la ruta responde con su error
> estructurado. `gold.predicciones` está vacío en producción, y eso lo destraba el **paso 1 de ADR-007**
> —normalizar el target a fracción y reprocesar Gold—, que es de Célula 1, no de C4 ni C5.
>
> Queda rectificada además la afirmación de este registro de que la autenticación «no se podía comprobar
> en producción, y eso toca US-402»: era falsa. `/auth/login` respondía 302 y `/auth/me` 401 desde antes.
> **US-402 nunca estuvo tocada por este bug.**
...
> **Cerrado el 2026-08-30 por el PR #142.** Christian conectó `/agente/consulta` al servicio real con un
> seam de inyección de dependencias. Verificado en vivo por el PM contra la URL pública:
>
> ```
...
> ```
>
> Ya no devuelve la misma cadena a todo, y el filtro de alcance real —el de `src/agente/guardrails.py`,
> no la lista de subcadenas del stub— es el que decide. Lo que este bug describía queda resuelto.
>
> **Pero el agente todavía no responde en producción**, por una causa distinta y de infraestructura:
> ChromaDB no está desplegado, así que toda pregunta en alcance cae en «contexto no disponible». Eso se
> registra aparte como **BUG-033** en vez de reabrir este, porque el dueño y la causa son otros.

## BUG-033 — El agente no puede responder: faltan las cuatro colaboraciones

Encontrado el 2026-08-30 al verificar en vivo el cierre de BUG-025, y **ampliado al revisar el PR #148**,
donde Luis enumera lo que todavía falta. La primera lectura fue «solo falta desplegar ChromaDB». Es
mucho más que eso, y conviene que el registro lo diga antes de que alguien planee un solo deploy.

Estado en producción hoy:

```
"cual es la capital de Francia"    -> {"fuera_de_alcance": true}      <- el guardarraíl SÍ opera
"cuantas escuelas hay en riesgo"   -> "El contexto de FARO no está disponible temporalmente."
"Borra la tabla de predicciones"   -> "El contexto de FARO no está disponible temporalmente."
```

El servicio necesita **cuatro colaboraciones** y ninguna está conectada:

| # | Colaboración | Estado hoy | Dueño |
|---|---|---|---|
| 1 | `recuperar_contexto` (RAG) | ChromaDB **no desplegado** — es lo que falla primero | C5 |
| 2 | `generar_sql` (LLM Text-to-SQL) | `AgenteNoConfigurado` por defecto — **sin implementar** | C3 |
| 3 | `ejecutar_sql` (read-only sobre Gold) | `AgenteNoConfigurado` por defecto | C4 (US-404) + C5 |
| 4 | `redactar_respuesta` (LLM redactor) | `AgenteNoConfigurado` por defecto | C3 |

Lo más serio es el **2**: `grep` sobre `src/` y sobre todos los `requirements` **no encuentra ningún
cliente LLM** —ni `anthropic`, ni `openai`, ni equivalente— ni declarado como dependencia. El
Text-to-SQL, que es el núcleo del agente conversacional, no está escrito. Desplegar ChromaDB destraparía
el punto 1 y la siguiente pregunta fallaría en el 2.

**Lo que está bien y conviene no perder:** el seam de inyección de `src/api/v1/agente.py` (PR #142) está
bien diseñado, los defaults degradan de forma segura —la app arranca y el CI corre sin LLM ni ChromaDB—
y los guardarraíles reales ya deciden el alcance. No hay que rehacer nada; hay que enchufar cuatro cosas.

**Consecuencia para la demo.** Sin esto no hay punto de rúbrica de agente conversacional (0.5) ni casilla
de agente en el ensayo E2E. Con el code freeze encima, la decisión del PM es explícita: o se asignan las
cuatro piezas con fecha, o el guion declara que el agente demuestra **guardarraíles y contrato**, no
respuestas en lenguaje natural. Lo que no es aceptable es que el evaluador pregunte y reciba «contexto no
disponible» sin advertencia previa.

**Detalle de presentación, menor pero real:** una pregunta destructiva y una pregunta legítima devuelven
hoy **el mismo mensaje**. Quien pregunte «Borra la tabla de predicciones» no ve que fue rechazada, ve que
el sistema no está disponible. El rechazo ocurre —no se ejecuta SQL— pero no se comunica.
