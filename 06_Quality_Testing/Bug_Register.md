...
| BUG-025 | El endpoint desplegado `/api/v1/agente/consulta` es el **stub** de `src/api/v1/agente.py`: responde **la misma cadena fija a cualquier pregunta**, incluidas las fuera de alcance y las destructivas. Además su filtro de palabras busca `"borrar"` por subcadena, así que **«Borra la tabla de predicciones» no lo dispara** y recibe la respuesta normal con `fuera_de_alcance: false`. Los guardarraíles reales de `src/agente/guardrails.py` —que sí rechazan esa frase— nunca se invocan desde la API | high | **fixed** | US-304a / US-305 / REQ-006 | **Christian Ruiz (C4), PR #142** — `/api/v1/agente/consulta` deja de ser el stub y conecta con el servicio RAG real, así que los guardarraíles de `src/agente/guardrails.py` por fin se invocan desde la API desplegada | `tests/test_agente_endpoint.py` + contrato actualizado en `tests/test_api_contract.py` |
